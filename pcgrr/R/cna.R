...
#' @param cna_segments data frame with annotated copy number segments
#'
#' @export
#'
plot_cna_segments <- function(chrom_coordinates = NULL,
  ...
  invisible(assertthat::assert_that(
    ...
    !is.null(cna_segments)
  ))

  invisible(assertthat::assert_that(
    base::is.data.frame(cna_segments),
  ...
  reference_coordinates <- chrom_coordinates |>
    ...
    dplyr::mutate(
      centromere_genome_start = .data$genome_start + .data$centromere_left,
      centromere_genome_end = .data$genome_start + .data$centromere_right,
      midpoint = round((.data$genome_start + .data$genome_end) / 2))
  ...
  onc_gain_tsg_loss <- cna_segments |>
    dplyr::select(
    ...
    dplyr::filter(
      (.data$ONCOGENE == TRUE & .data$VARIANT_CLASS == "gain") |
        (.data$TUMOR_SUPPRESSOR == TRUE & .data$VARIANT_CLASS == "loss"))
    ...
    if(NROW(onc_gain) > 0){
      onc_gain <- onc_gain |>
        ...
        dplyr::group_by(.data$CHROM,
                        .data$SEGMENT_START,
                        .data$SEGMENT_END) |>
        dplyr::summarise(ONC_GAIN = paste(utils::head(.data$SYMBOL, 3), collapse = ", "),
                         .groups = "drop")
    }

    tsg_loss <- onc_gain_tsg_loss |>
      dplyr::filter(.data$TUMOR_SUPPRESSOR == TRUE & .data$VARIANT_CLASS == "homdel")
    ...
    if(NROW(tsg_loss) > 0){
      tsg_loss <- tsg_loss |>
        ...
        dplyr::group_by(.data$CHROM,
                        .data$SEGMENT_START,
                        .data$SEGMENT_END) |>
        dplyr::summarise(TSG_LOSS = paste(utils::head(.data$SYMBOL, 3), collapse = ", "),
                         .groups = "drop")
    }
  ...
  cna_segments_global <- cna_segments |>
    dplyr::select(
      c("CHROM", "SEGMENT_START", "SEGMENT_END",
        "CYTOBAND","EVENT_TYPE","CN_MINOR",
        "CN_TOTAL", "VARIANT_CLASS")) |>
    dplyr::mutate(CHROM = paste0("chr", .data$CHROM)) |>
    dplyr::left_join(
      dplyr::select(
    ...
    dplyr::mutate(
      segsize = round((.data$SEGMENT_END - .data$SEGMENT_START) / 1000000, digits = 3)) |>
    ...
    dplyr::select(
      -c("genome_start","EVENT_TYPE","VARIANT_CLASS","segsize")) |>
    dplyr::distinct()
  ...
  if(NROW(onc_gain) > 0){
    cna_segments_global <- cna_segments_global |>
      ...
      dplyr::mutate(SegmentInfo = dplyr::if_else(
        !is.na(.data$ONC_GAIN),
        paste0(.data$SegmentInfo, "<br> - Oncogene gain: ", .data$ONC_GAIN),
        .data$SegmentInfo))
  }else{
