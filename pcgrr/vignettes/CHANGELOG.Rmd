...
- Date: **2026-08-10**
- Patch release to fix [issue #300](https://github.com/sigven/pcgr/issues/300) and
  [issue #302](https://github.com/sigven/pcgr/issues/302)

## v2.3.1

- Date: **2026-07-10**
- Patch release to fix a bug with samtools version for vcf2maf (`r issue(297)`)
...
- **OncoKB clinical evidence integration**
  - Real-time retrieval of therapeutic, diagnostic, and prognostic evidence from the
    [OncoKB MAF annotator + web API](https://www.oncokb.org) for somatic variants - requires a valid API
	token (see new CLI argument `--oncokb_api_token`)
  - Evidence is retrieved for SNV/InDels (via HGVSp protein change and HGVSg genomic
    change), gene fusions, and copy number alterations
  - Tumor type context is passed to OncoKB via OncoTree codes, enabling tumor-specific
    evidence retrieval for both exact and any-tumor-type matches
  - OncoKB annotation includes mutation effect (e.g. Gain-of-function), oncogenicity
    (e.g. Oncogenic, Likely Oncogenic), and clinical evidence items
    (treatment sensitivity/resistance, diagnostic and prognostic implications)
  - New CLI arguments:
    - `--oncokb_api_token` — a valid [OncoKB API token](https://www.oncokb.org/account/settings)
      is required to enable this feature
    - `--oncokb_oncotree_code` — override the tumor-type OncoTree code used for OncoKB
      queries (default: derived from `--tumor_site`); useful when the desired OncoTree
      code is more specific than the site-level mapping
    - `--oncokb_exclusive` — restrict biomarker reporting to OncoKB only, skipping CIViC
      and CGI sources; intended for workflows where a single curated source is preferred
    - `--oncokb_maf_query_all` - query OncoKB with all variant types in MAF (no
      filtering of non-coding variants (IGR, Intron, UTR), which is the default)
- **Copy number classification — flexible thresholding modes**
  - New CLI argument `--cna_threshold_mode` controls how amplification, gain, and
    heterozygous deletion tiers are called; three modes are supported:
    - `absolute` _(default)_: thresholds are applied to the total copy number directly
    - `relative`: thresholds are expressed as fold-change relative to tumor ploidy
    - `combined`: both absolute and relative thresholds must be satisfied
  - Per-tier, per-mode thresholds are individually configurable:
    ...
    - Heterozygous deletion: `--cna_del_threshold_absolute` (default: 1),
      `--cna_del_threshold_relative` (default: 0.5×)
  - Allele-specific copy numbers (major + minor) are used for correct handling of
    loss of heterozygosity (LOH) and sex-chromosome dosage
  - If tumor ploidy is not provided by the user, PCGR estimates it from the
    allele-specific copy number distribution as a fallback (used by `relative` and
    `combined` modes)
  - Two-hit candidate annotation: segments overlapping a known tumor suppressor gene
    are flagged when both a somatic loss-of-function SNV/InDel _and_ a CNA deletion
    event are present (requiring CPSR input for germline hit detection)
- **RNA gene fusion input**
  - New CLI argument `--input_rna_fusion` accepts a TSV file with RNA fusion calls
    (e.g. from STAR-Fusion, Arriba, or similar callers)
  - Fusions are displayed in a dedicated section of the HTML report, annotated with
    gene-level oncogenicity and actionability
  - If an OncoKB API token is provided, fusions are queried against the OncoKB API for
    clinical evidence (therapeutic, diagnostic, prognostic) and mutation effect
- **Reactable-based interactive tables throughout HTML report**
  - All interactive variant and biomarker evidence tables now use
    [reactable](https://glin.github.io/reactable/) in place of the previous DT/DataTables
    implementation
  - Tables support column-level filtering, global search, and compact nested detail rows
    (expandable per-variant evidence panels) — providing a more responsive and
    feature-rich browsing experience
  - A hidden `SEARCH_INDEX` column concatenates key fields (gene, alteration, consequence,
    oncogenicity) to enable full-text search across the most clinically relevant columns
- **Excel workbook — SETTINGS and DATA_VERSIONS sheets**
  - Two new sheets are prepended to the Excel workbook output (`.xlsx`), before
    ...
    - `SETTINGS` — key run parameters in a three-column `SECTION / PARAMETER / VALUE`
      layout, covering: general settings (PCGR version, genome assembly, biomarker
      sources, guidelines), SNV/InDel settings (noncoding display, germline findings,
      MSI, TMB algorithm, mutational signature sub-settings), VEP settings (transcript
      set, pick order, regulatory annotation, buffer size, forks), allelic support
      filters (only rows for filters that are actively configured), CNA thresholding
      settings (all absolute/relative thresholds for amplification, gain, and
      heterozygous deletion), RNA fusion settings (min split reads), and expression
      settings (similarity analysis)
...
- **MSI prediction confidence**
  - The MSI random forest classifier now exposes calibrated class probabilities
    (P(MSI-H) from `randomForest::predict(..., type = "prob")`) alongside the
    hard MSI-H / MSS call
  - A confidence tier is derived directly from the predicted probability:
    samples with P(MSI-H) ≥ 0.9 or ≤ 0.1 are labelled **High confidence**;
    those in the 0.1–0.9 range are labelled **Uncertain** — reflecting the
    well-calibrated, strongly bimodal score distribution observed on the TCGA
    training/holdout set (Brier score ~0.011)
  - Both values are surfaced in:
    - **HTML report** — the MSI value box shows the predicted probability and
      confidence tier alongside the classification label (e.g. "0.97 — High confidence")
...
- Variant tier classification with respect to actionability is now driven by treatment
  sensitivity evidence only. While treatment resistance, diagnostic and prognostic evidence 
  linked to query variants are shown, they do not contribute to tier assignment - this to align
  most correctly with the AMP/ASCO/CAP guidelines for variant actionability assessment.
- Fix to implementation of oncogenicity classification: OS3 does not fire when OS1 is already
  firing (known oncogenic/likely oncogenic variant)
- Loss-of-function annotation from MaxEntScan is now stratum-aware: canonical splice
  site positions (±2 bp of exon–intron boundary) always set `LOSS_OF_FUNCTION = TRUE`;
  for non-canonical positions, only variants with a _Strong_ MaxEntScan evidence tier
  (donor positions +3 to +6; acceptor positions -3 to -20) additionally set
  `LOSS_OF_FUNCTION = TRUE` — _Moderate_ and _Supporting_ tiers at these positions are
  informational only
- Tumor-only germline filtering now uses a unified per-variant gnomAD allele frequency
  flag (`gnomAD_AF_ABOVE_TOLERATED`) that considers all available population frequencies
  from both exome and genome gnomAD datasets, preferring exome values when available
- CLI option `--vcf2maf` is no longer an argument to PCGR - vcf to MAF conversion will   run automatically as part of the workflow when a VCF input is provided.

### Infrastructure

- Major refactor of GitHub Actions workflow (`r pr(291)`, `r issue(247)`, `r issue(248)`, `r issue(262)`)
  ...
  - switch to rattler-build from conda mambabuild
    - also requires moving to the required rattler format for the conda recipes
...
- Add oncokb-annotator conda dependency (https://github.com/oncokb/oncokb-annotator) - https://anaconda.org/channels/pcgr/packages/oncokb-annotator/files
- Updated vcf2maf to 1.6.22 (see https://github.com/mskcc/vcf2maf/releases/tag/v1.6.22 for bug fixes)
...
- cleaned up plotting code (variant statistics, mutational signatures)
  - reduce opacity for pie chart categories that are being filtered (tumor-only runs)
...
- fixed bug in site to TCGA cohort matching (`r issue(275)`)
- fixed bug in copy number segment annotations with no protein-coding overlap (`r issue(278)`)
- added gene `loss` biomarkers (loss-of-function) as records to match for CNA homozygous deletions (`r issue(277)`)
- added new option `--sex` to enable sex-adjusted annotation of CNA segments (i.e. hemizygous deletions for males on chrX)
...
- update Ensembl VEP `v113.4` (see [bioconda issue](https://github.com/bioconda/bioconda-recipes/pull/54330))
...
- fix bug in Grantham distance score evaluation (`r issue(271)`)
- add support for heterozygous deletions in CNA interpretation - variant classes
  are now _homdel_, _hetdel_, and _gain_
