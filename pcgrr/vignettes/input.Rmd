...
The PCGR workflow accepts three main input files:
  ...
  * An allele-specific copy number segment file
  * A file with transcript/gene expression levels
...
In addition to these three main input files, the user can also opt to provide a [panel-of-normals VCF](#panel-of-normals-pon-vcf) file, as well as a [CPSR report (JSON)](#cpsr-report), as input.
...
### Germline variant findings

One may feed classified variant calls from a cancer predisposition analysis with CPSR (https://github.com/sigven/cpsr) into PCGR through the `--input_germline` option. Specifically, CPSR
produces a tab-separated output file (`<sample_id>.cpsr.<assembly>.classification.tsv.gz`) that
should be used as the value for `--input_germline`. This will generate a dedicated section on
germline findings in the PCGR report, which in turn will allow the user to interrogate somatic findings
in the context of potential cancer-predisposing germline variants for the individual analyzed. 
