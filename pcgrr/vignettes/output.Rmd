...
| 41. `ONCOGENICITY_OKB` | OncoKB oncogenicity annotation (e.g. Oncogenic, Likely Oncogenic) |
| 42. `CANONICAL` | Flag indicating if transcript is canonical |
| 43. `CCDS` | CCDS identifier |
| 44. `UNIPROT_ACC` | UniProt accession |
| 45. `ENSEMBL_TRANSCRIPT_ID` | Ensembl transcript identifier |
| 46. `ENSEMBL_PROTEIN_ID` | Ensembl protein identifier |
| 47. `REFSEQ_TRANSCRIPT_ID` | RefSeq transcript identifier |
| 48. `REFSEQ_PROTEIN_ID` | RefSeq protein identifier |
| 49. `MANE_SELECT` | MANE transcript select |
| 50. `MANE_PLUS_CLINICAL` | MANE transcript plus clinical |
| 51. `ONCOGENE` | Flag indicating if gene is oncogene (CancerMine/NCG) |
| 52. `ONCOGENE_SUPPORT` | Oncogene annotation support (CancerMine/NCG) |
| 53. `TUMOR_SUPPRESSOR` | Flag indicating if gene is tumor suppressor (CancerMine/NCG) |
| 54. `TUMOR_SUPPRESSOR_SUPPORT` | Tumor suppressor annotation support (CancerMine/NCG) |
| 55. `TARGETED_INHIBITORS2` | Targeted inhibitors |
| 56. `REGULATORY_ANNOTATION` | Regulatory annotation |
| 57. `VEP_ALL_CSQ` | VEP consequence - all transcripts |
| 58. `gnomADe_AF` | gnomAD exomes allele frequency - globally |
| 59. `gnomADg_AF` | gnomAD genomes allele frequency - globally |
| 60. `DBSNP_RSID` | dbSNP identifier |
| 61. `COSMIC_ID` | COSMIC identifier |
| 62. `TCGA_FREQUENCY` | Frequency of variant across TCGA tumor types. Format: `tumortype | percent affected | affected cases | total cases` |
| 63. `TCGA_PANCANCER_COUNT` | Raw variant count across all TCGA tumor types |
| 64. `CLINVAR_MSID` | ClinVar measureset identifier |
| 65. `CLINVAR_CLASSIFICATION` | ClinVar variant classification |
| 66. `CLINVAR_VARIANT_ORIGIN` | ClinVar variant origin |
| 67. `CLINVAR_NUM_SUBMITTERS` | ClinVar number of submitters |
| 68. `CLINVAR_GOLD_STARS` | ClinVar review status gold stars (0-4) |
| 69. `CLINVAR_CONFLICTED` | ClinVar variant classification is conflicted |
...
| 14. `TRANSCRIPT_OVERLAP_PERCENT` | Percentage overlap between copy number segment and transcript |
| 15. `CYTOBAND` | Associated cytoband |
| 16. `EVENT_TYPE` | Focal or broad (covering more than 25% of chromosome arm) |
| 17. `VARIANT_CLASS` | _amplification_: total copy number >= user-defined threshold; _homdel_: total copy number equal to zero; _hetdel_: total copy number equal to one; _gain_: copy number gain below amplification threshold; _undefined_: other copy number states |
| 18. `VARIANT_CLASS_DISPLAY` | Display label for variant class |
| 19. `SYMBOL` | Gene symbol |
| 20. `ENTREZGENE` | Entrez gene identifier |
| 21. `GENENAME` | Gene name |
| 22. `ENSEMBL_GENE_ID` | Ensembl gene identifier |
| 23. `TUMOR_SUPPRESSOR` | Flag indicating if gene is tumor suppressor (CancerMine/NCG) |
| 24. `TUMOR_SUPPRESSOR_SUPPORT` | Tumor suppressor annotation support (CancerMine/NCG) |
| 25. `ONCOGENE` | Flag indicating if gene is oncogene (CancerMine/NCG) |
| 26. `ONCOGENE_SUPPORT` | Oncogene annotation support (CancerMine/NCG) |
| 27. `MUTATION_EFFECT_OKB` | OncoKB mutation effect annotation (e.g. Loss-of-function, Gain-of-function) |
| 28. `ONCOGENICITY_OKB` | OncoKB oncogenicity annotation (e.g. Oncogenic, Likely Oncogenic) |
| 29. `TRANSCRIPT_OVERLAP` | Comma-separated list of associated transcripts, including percentage of transcript overlap |
| 30. `ACTIONABILITY_TIER` | Variant actionability tier - AMP/ASCO/CAP |
| 31. `ACTIONABILITY` | Variant clinical actionability significance - AMP/ASCO/CAP |
| 32. `BIOMARKER_MATCH` | Biomarker match |
| 33. `TARGETED_INHIBITORS_ALL2` | Molecularly targeted inhibitors - indicated for any tumor type |
