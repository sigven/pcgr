...
| 10. `CDS_CHANGE` | Protein change |
| 11. `HGVSP` | Protein change in one-letter amino-acid HGVS notation, commonly referred to as HGVSp_short |
| 12. `EXON` | Exon number (out of total number of exons) |
| 13. `CONSEQUENCE` | Variant consequence - from VEP |
| 14. `PFAM_DOMAIN_NAME` | Pfam domain name |
| 15. `LOSS_OF_FUNCTION` | Loss of function flag |
| 16. `LOF_FILTER` | Loss of function filter |
| 17. `CODING_STATUS` | Coding status - flag indicating if consequence is protein-altering/affecting splice sites|
| 18. `EXONIC_STATUS` | Exonic status - flag indicating if consequence is silent/protein-altering/affecting splice sites|
| 19. `DP_TUMOR` | Depth of coverage at variant position in tumor sample |
| 20. `VAF_TUMOR` | Variant allele fraction at variant position in tumor sample |
| 21. `AD_TUMOR` | Allelic depth (number of reads supporting alt allele) in tumor sample |
| 22. `DP_CONTROL` | Depth of coverage at variant position in control sample |
| 23. `VAF_CONTROL` | Variant allele fraction at variant position in control sample |
| 24. `AD_CONTROL` | Allelic depth (number of reads supporting alt allele) in control sample |
| 25. `MUTATION_HOTSPOT` | Mutation hotspot annotation |
| 26. `MUTATION_HOTSPOT_CANCERTYPE` | Mutation hotspot-associated cancer types (from cancerhotspots.org) |
| 27. `ACTIONABILITY_TIER` | Variant clinical actionability tier - AMP/ASCO/CAP implementation |
| 28. `ACTIONABILITY` | Variant clinical actionability significance - AMP/ASCO/CAP implementation |
...
| 40. `ENSEMBL_PROTEIN_ID` | Ensembl protein identifier |
| 41. `REFSEQ_TRANSCRIPT_ID` | RefSeq transcript identifier |
| 42. `REFSEQ_PROTEIN_ID` | RefSeq protein identifier |
| 43. `MANE_SELECT` | MANE transcript select |
| 44. `MANE_PLUS_CLINICAL` | MANE transcript plus clinical |
| 45. `ONCOGENE` | Flag indicating if gene is oncogene (CGC/CancerMine/NCG) |
| 46. `ONCOGENE_SUPPORT` | Oncogene annotation support (CancerMine/NCG) |
| 47. `TUMOR_SUPPRESSOR` | Flag indicating if gene is tumor suppressor (CancerMine/NCG) |
| 48. `TUMOR_SUPPRESSOR_SUPPORT` | Tumor suppressor annotation support (CancerMine/NCG) |
...
| 55. `gnomADg_AF` | gnomAD genomes allele frequency - globally |
| 56. `DBSNP_RSID` | dbSNP identifier |
| 57. `COSMIC_ID` | COSMIC identifier |
| 58. `TCGA_FREQUENCY` | Frequency of variant across TCGA tumor types. Format: `tumortype | percent affected | affected cases | total cases` |
| 59. `TCGA_PANCANCER_COUNT` | Raw variant count across all TCGA tumor types |
| 60. `CLINVAR_MSID` | ClinVar measureset identifier |
| 61. `CLINVAR_CLASSIFICATION` | ClinVar variant classification |
| 62. `CLINVAR_VARIANT_ORIGIN` | ClinVar variant origin |
| 63. `CLINVAR_NUM_SUBMITTERS` | ClinVar number of submitters |
| 64. `CLINVAR_REVIEW_STATUS_STARS` | ClinVar number of review status stars |
| 65. `CLINVAR_CONFLICTED` | ClinVar variant classification is conflicted |
| 66. `BIOMARKER_MATCH` | Biomarker match |
| 67. `CALL_CONFIDENCE` | Call confidence |
...
variant if it is classified as somatic or germline. Specifically, the value of `SOMATIC_CLASSIFICATION` can be either *SOMATIC* (not catched by any filter), or containing one or more of the following tags pending on which sources that classified the variant as likely germline (and whether these filters were activated by the user):
*GERMLINE_GNOMAD* (always turned on), *GERMLINE_CLINVAR*, *GERMLINE_DBSNP*, *GERMLINE_HET*, *GERMLINE_HOM*, *GERMLINE_PON*
...
| 8. `VARIANT_CLASS` | _amplification_: total copy number >= user-defined threshold; _homdel_ - total copy number equal to zero; _hetdel_ - total copy number equal to one; _undefined_ other copy number states|
| 9. `SYMBOL` | Gene symbol |
| 10. `ENTREZGENE` | Entrez gene identifier |
| 11. `GENENAME` | Gene name |
| 12. `ENSEMBL_GENE_ID` | Ensembl gene identifier |
| 13. `TUMOR_SUPPRESSOR` | Flag indicating if gene is tumor suppressor (CancerMine/NCG) |
| 14. `TUMOR_SUPPRESSOR_SUPPORT` | Tumor suppressor annotation support (CancerMine/NCG) |
| 15. `ONCOGENE` | Flag indicating if gene is oncogene (CancerMine/NCG) |
| 16. `ONCOGENE_SUPPORT` | Oncogene annotation support (CancerMine/NCG) |
...
| 20. `BIOMARKER_MATCH` | Biomarker match |
| 21. `TARGETED_INHIBITORS_ALL2` | Molecularly targeted inhibitors - indicated for any tumor type|
