...
The following oncogenicity criteria are currently used for such classification in PCGR:

 * _ONCG_SBVS1_ - Very high MAF (> 0.05 in gnomAD - any five major continental populations)
 * _ONCG_SBS1_ - High MAF (> 0.01 in gnomAD - any five major continental populations)
 * _ONCG_SBP1_ - _Insilico_ support for a benign effect on the gene or gene product (multiple lines of evidence (>= 8 algorithms) from dbNSFP)
 * _ONCG_SBP2_ - Silent and intronic changes outside of the consensus splice site (VEP consequence)
 * _ONCG_OVS1_ - Null variant - in a _bona fide_ tumor suppressor gene (predicted as LoF in tumor suppressors from CGC/NCG/CancerMine)
 * _ONCG_OS1_ - Same amino acid change as previously established oncogenic variant (ClinVar as underlying source)
 * _ONCG_OS3_ - Located in a mutation hotspot, >= 50 samples with a variant at amino acid position, >= 10 samples with same amino acid change (cancerhotspots.org)
 * _ONCG_OM1_ - Located in a presumably critical site of functional domain - here, this is implemented through indirect evidence from overlap with known predictive (drug sensitivity/resistance) biomarker loci 
 * _ONCG_OM2_ - Protein length changes from in-frame dels/ins in known oncogene/tumor suppressor genes or stop-loss variants in a tumor suppressor gene (tumor suppressors/oncogenes from CGC/NCG/CancerMine)
 * _ONCG_OM3_ - Missense variant at an amino acid residue where a different missense variant determined to be oncogenic has been documented (ClinVar as underlying source). Grantham distance must be approximately the same as the known oncogenic variant
 * _ONCG_OM4_ - Located in a mutation hotspot, < 50 samples with a variant at amino acid position, >= 10 samples with same amino acid change (cancerhotspots.org)
 * _ONCG_OP1_ - _Insilico_ support for a damaging effect on the gene or gene product (multiple lines of evidence (>= 8 algorithms) from dbNSFP)
 * _ONCG_OP3_ - Located in a mutation hotspot, < 10 samples with the same amino acid change (cancerhotspots.org)
 * _ONCG_OP4_ - Absent from controls (gnomAD) / very low MAF ( < 0.0001 in all five major subpopulations)
