...
if(NROW(pcg_report$content$mutational_signatures$variant_set$all) < 100){
...
if(!missing_data_norun){
  ...
  flexdashboard::valueBox(
    "ND", color = pcgrr::color_palette$none)
...
if(num_events > 0){
  num_events <- 
    NROW(pcg_report$content$kataegis$events |> 
           dplyr::filter(confidence == 3))
}
flexdashboard::valueBox(
  num_events, color = ifelse(
    num_events > 0,
    pcgrr::color_palette$success,
...
names(scale_fill) <- 
  pcg_report$content$mutational_signatures$result$scale_fill_names
...
cat(paste0('-   <b>NOT DETERMINED (ND)</b>: The number of SNV calls (n = ',NROW(pcg_report$content$mutational_signatures$variant_set$all),') is too limited. A minimum of ',pcg_report$settings$conf$somatic_snv$mutational_signatures$mutation_limit,' SNVs is currently required for a mutational signature analysis to be performed.'), sep ="\n")
...
myOptions <- list(
  ...
  scrollCollapse = T)
dat <- dplyr::select(
  pcg_report$content$mutational_signatures$result$contributions$per_signature, 
  -c(sample_id,col,prop_signature)) |>
  dplyr::select(signature_id, contribution,
...
plot <- ggplot2::ggplot(
  rfdat$data, ggplot2::aes(x = location, y = distance)) +
    ggplot2::geom_point(
      ggplot2::aes(colour = factor(type)), cex = rfdat$cex) +
    ggplot2::geom_vline(
