...
PCGR can estimate TMB in different manners, depending on the type of variants to include in the calculation (specifically abbreviated as __coding_non_silent__, __coding_and_silent__, and __missense_only__, see details outlined in the [Documentation](#documentation)). Only results from one of these methods are highlighted here, as configured by the user. 
...
if ((pcg_report$settings$conf$sample_properties$dp_tumor_detected &
    pcg_report$settings$conf$somatic_snv$tmb$tmb_dp_min > 0) |
   (pcg_report$settings$conf$sample_properties$vaf_tumor_detected &
    pcg_report$settings$conf$somatic_snv$tmb$tmb_af_min > 0)) {
  cat("For TMB calculation, requirements for sequencing depth and/or variant allele frequency (VAF) have been applied to filter out low-quality variants\n\n")
}else{
  cat("No dedicated filtering of variants based on sequencing depth or variant allele frequency (VAF) has been applied for TMB calculation\n\n")
}
...
* The TCGA somatic mutation calls used for TMB calculation have been filtered for sequencing depth (minimum DP=30), and variant allele frequency (minimum VAF=0.05)
...
## Highlight color used for the query sample's primary site in the plot
## differs by assay type (color_palette$report_color) - name it correctly
## in the caption rather than hardcoding a single color
tmb_highlight_color_name <- if (isTRUE(as.logical(
  pcg_report$settings$conf$assay_properties$vcf_tumor_only))) {
  "blue"
} else {
  "purple"
}
...
bslib::card(
    ...
    bslib::card_footer(
      ...
      paste0(
        "Each row shows the distribution of tumor mutational burden (TMB, log10 mutations/Mb) ",
        "among TCGA samples of a given primary tumor site (box: interquartile range; violin: ",
        "full distribution shape), sorted by descending median TMB. The row highlighted in ",
        tmb_highlight_color_name, " corresponds to the query sample's primary site (",
        pcg_report$settings$conf$sample_properties$site,
        "). The dashed vertical line marks this sample's own TMB estimate for comparison ",
        "across all sites. Note: the TMB axis is log10-scaled - axis labels indicate the ",
        "power of ten (e.g. \"1\" = 10 mutations/Mb)."
      )
    )
  )
```
