...
if(NROW(pcg_report[['content']][['snv_indel']][['variant_set']][['tsv']]) > 0){
  if(NROW(dplyr::filter(pcg_report[['content']][['snv_indel']][['variant_set']][['tsv']], !is.na(DP_TUMOR))) > 0 & NROW(dplyr::filter(pcg_report[['content']][['snv_indel']][['variant_set']][['tsv']], !is.na(VAF_TUMOR))) > 0){
    tumor_af_plot <- T
  }
}
```
...
if(any(!is.na(vars$DP_TUMOR)) & any(!is.na(vars$VAF_TUMOR))){
  ...
  filter_set2[[i_set2]] <- crosstalk::filter_slider("VAF_TUMOR", "Allelic fraction tumor", variants_shared, ~VAF_TUMOR, 
