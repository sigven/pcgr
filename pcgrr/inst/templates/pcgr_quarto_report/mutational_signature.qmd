...
if (msig_content$eval == F | 
   msig_content$missing_data == TRUE) {
...
#| eval: !expr isFALSE(missing_data_no_refit) & isTRUE(poor_sig_fit)
#| output: asis
...
#| eval: !expr isFALSE(missing_data_no_refit)
#| output: asis
...
pie_aetiologies <- plotly::plot_ly(
  ...
  #margin = m,
  marker = list(
    colors = pcgrr::color_palette$multi$values,
    line = list(
    ...
    bslib::card(
      ...
      bslib::card_header(
        ...
        paste0("Key underlying aetiologies - ",
               pcg_report$settings$sample_id)),
      bslib::card_body(padding = "4px", pie_aetiologies)
    )
  )
)
```
...
#| eval: !expr isFALSE(is.null(msig_content$result$signature_similarity))
#| child: pcgr_quarto_report/mutational_signatures/signature_similarity.qmd
```

```{r signature_similarity_missing}
#| eval: !expr isTRUE(is.null(msig_content$result$signature_similarity))
#| output: asis
