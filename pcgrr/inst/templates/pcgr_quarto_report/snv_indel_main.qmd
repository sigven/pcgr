...
Annotations for all short variants detected in the tumor sample can here be interrogated, e.g. with respect to VAF/DP (if provided as input), and also with respect to _oncogenicity_. For the latter, PCGR implements recently published oncogenicity criteria/guidelines developed by ClinGen/VICC [@Horak2022-uh]. The tabset below provides an overview of coding (protein-altering) versus non-coding variants found in the tumor sample.
...
_clinical actionability_ through an implementation of ACMG/AMP guidelines [@Li2017-ew]. The tabset below indicates which variants have been classified into three distinct _tiers of clinical significance_ according to ACMG/AMP criteria.
...
#### <b>TIER 3</b> - uncertain clinical significance

Here, we list protein-altering somatic variants located in proto-oncogenes and tumor suppressors that are currently not linked to any clinical evidence items. These variants are considered to have uncertain clinical significance and are thus not recommended for clinical action.
