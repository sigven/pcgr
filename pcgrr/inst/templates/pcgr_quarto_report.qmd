---
title: "<SAMPLE_NAME>"
subtitle: " - _cancer genome interpretation report by PCGR_ <ASSAY_BUILD_BADGES>"
...
knitr::opts_chunk$set(echo = FALSE,warning=FALSE, dpi=72, error=F, eval=T)
...
is_tumor_only <- as.logical(pcg_report$settings$conf$assay_properties$vcf_tumor_only)
...
#| eval: !expr isTRUE(pcg_report$content$snv_indel$eval)
...
#| eval: !expr isTRUE(pcg_report$content$cna$eval)

```

```{r pcgr_rna_fusion}
#| child: 'pcgr_quarto_report/rna_fusion.qmd'
#| eval: !expr isTRUE(pcg_report$content$fusion$eval)

```

```{r pcgr_signatures}
#| child: 'pcgr_quarto_report/mutational_signature.qmd'
#| eval: !expr isTRUE(pcg_report$content$mutational_signatures$eval)

```

```{r pcgr_kataegis}
#| child: 'pcgr_quarto_report/kataegis.qmd'
#| eval: !expr isTRUE(pcg_report$content$kataegis$eval)

```

```{r pcgr_msi}
#| child: 'pcgr_quarto_report/msi.qmd'
#| eval: !expr isTRUE(pcg_report$content$msi$eval)

```

```{r pcgr_tmb}
#| child: 'pcgr_quarto_report/mutational_burden.qmd'
#| eval: !expr isTRUE(pcg_report$content$tmb$eval)

```

```{r pcgr_germline}
#| child: 'pcgr_quarto_report/germline.qmd'
#| eval: !expr isTRUE(pcg_report$content$germline_classified$eval)

```

```{r pcgr_expression}
#| child: 'pcgr_quarto_report/expression.qmd'
#| eval: !expr isTRUE(pcg_report$content$expression$eval)
