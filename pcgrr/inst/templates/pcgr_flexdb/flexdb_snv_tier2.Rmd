...
flexdashboard::valueBox(
  "TIER 2", caption = NULL, color = '#000000', icon = NULL)
```
...
```{r}
flexdashboard::valueBox(
  pcg_report$content$snv_indel$vstats$n_genes_tier2, 
...
```{r}
flexdashboard::valueBox(
  pcg_report$content$snv_indel$vstats$n_actionability_tier2, 
  color = pcg_report$settings$conf$report_color,
  icon = "fa-dna")
```
...
```{r}
flexdashboard::valueBox(
  pcg_report$content$snv_indel$vstats$n_eitems_diagnostic_tier2, 
  color = ifelse(
    pcg_report$content$snv_indel$vstats$n_eitems_diagnostic_tier2 > 0,
...
```{r}
flexdashboard::valueBox(
  pcg_report$content$snv_indel$vstats$n_eitems_prognostic_tier2, 
  color = ifelse(
    pcg_report$content$snv_indel$vstats$n_eitems_prognostic_tier2 > 0,
    pcgrr::color_palette$success,
    pcgrr::color_palette$none), 
  icon = "fa-file-prescription")
```
...
```{r}
flexdashboard::valueBox(
  pcg_report$content$snv_indel$vstats$n_eitems_predictive_tier2, 
  color = ifelse(
    pcg_report$content$snv_indel$vstats$n_eitems_predictive_tier2 > 0,
    pcgrr::color_palette$success,
    pcgrr::color_palette$none), 
  icon = "fa-file-prescription")
```
...
```{r table_browse_tier2, echo=F, results = "asis", eval = pcg_report$content$snv_indel$vstats$n_actionability_tier2 > 0}
...
var_eitems_tier2 <- 
  ...
  dplyr::filter(!is.na(.data$TIER) & TIER == 2) |>
...
var_eitems_tier2_shared <- crosstalk::SharedData$new(
  var_eitems_tier2)
...
```{r biomarker_note2, echo=F, results = "asis", include = pcg_report$content$snv_indel$vstats$n_actionability_tier2 > 0}
...
cat('<ul><li>Biomarker match at the <mark style="background-color:black; font-weight:bold; color:white">genomic, amino acid or codon level</mark></li>')
...
```{r tier2_missing_filters, echo=F, results = 'asis', eval = pcg_report$content$snv_indel$vstats$n_actionability_tier2 == 0}
...
```{r table_tier2, eval = pcg_report$content$snv_indel$vstats$n_actionability_tier2 > 0}
...
var_eitems_tier2_shared |>
  DT::datatable(
    ...
    options = list(
      ...
      scrollX = "900px",
      columnDefs = list(
        list(width = '90px', targets = c(1,2,3,4,5,6))),
                 dom = 'Bfrtip')) |>
  DT::formatStyle(
    'BM_EVIDENCE_LEVEL', 
