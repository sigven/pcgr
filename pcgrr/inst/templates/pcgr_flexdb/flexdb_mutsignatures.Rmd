...
if(pcg_report$content$snv_indel$vstats$n_snv < 100){
  col <- pcg_report$settings$conf$visual_reporting$color_palette[['warning']][['values']][1]
}
flexdashboard::valueBox(pcg_report$content$snv_indel$vstats$n_snv, color = col, icon = "fa-dna")
```
...
if(!missing_data_norun){
  m <- pcg_report$content$mutational_signatures$result$contributions$per_group |> 
    dplyr::arrange(desc(prop_group))
  flexdashboard::valueBox(m$group[1], color = pcg_report$settings$conf$visual_reporting$color_value_box)
}else{
  flexdashboard::valueBox("ND", color = pcg_report$settings$conf$visual_reporting$color_none)
...
if(!missing_data_norun){
  cosine_sim <- pcg_report$content$mutational_signatures$result$goodness_of_fit
...
flexdashboard::valueBox(num_events, color = ifelse(num_events > 0,
                        pcg_report$settings$conf$visual_reporting$color_palette[['success']][['values']][1],
                        pcg_report$settings$conf$visual_reporting$color_none))
```
...
cat(paste0('-   <b>NOT DETERMINED (ND)</b>: The number of SNV calls (n = ',nrow(pcg_report[['content']][['mutational_signatures']][['variant_set']][['all']]),') is too limited. A minimum of ',pcg_report[['metadata']][['config']][['msigs']][['mutation_limit']],' SNVs is currently required for a mutational signature analysis to be performed.'), sep ="\n")
...
MutationalPatterns::plot_96_profile(pcg_report$content$mutational_signatures$result$mut_mat, 
