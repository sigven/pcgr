...
### Cancer-predisposing variants (CPSR)

```{r, eval = T}
flexdashboard::valueBox("Germline findings", caption = NULL, color = '#000000', icon = NULL)
```

```{r prepare_cpsr_data, eval = T}
...
if(NROW(cpsr_rep$content$snv_indel$variant_set$class5) > 0){
  n_pathogenic <- cpsr_rep$content$snv_indel$variant_set$class5 %>%
    dplyr::filter(
      (!is.na(CPSR_CLASSIFICATION) & CPSR_CLASSIFICATION == "Pathogenic") |
        (!is.na(CLINVAR_CLASSIFICATION) & CLINVAR_CLASSIFICATION == "Pathogenic")) %>%
    NROW()
    
}

if(NROW(cpsr_rep$content$snv_indel$variant_set$class4) > 0){
  n_likely_pathogenic <- cpsr_rep$content$snv_indel$variant_set$class4 %>%
    dplyr::filter(
      (!is.na(CPSR_CLASSIFICATION) & CPSR_CLASSIFICATION == "Likely_Pathogenic") |
        (!is.na(CLINVAR_CLASSIFICATION) & CLINVAR_CLASSIFICATION == "Likely_Pathogenic")) %>%
    NROW()
    
}

if(NROW(cpsr_rep$content$snv_indel$variant_set$class3) > 0){
  n_vus <- cpsr_rep$content$snv_indel$variant_set$class3 %>%
    dplyr::filter(
      (!is.na(CPSR_CLASSIFICATION) & CPSR_CLASSIFICATION == "VUS") |
        (!is.na(CLINVAR_CLASSIFICATION) & CLINVAR_CLASSIFICATION == "VUS")) %>%
    NROW()
...
flexdashboard::valueBox(n_pathogenic, color = dplyr::if_else(
  n_pathogenic > 0,
...
flexdashboard::valueBox(n_likely_pathogenic, color = dplyr::if_else(
  n_likely_pathogenic > 0,
  pcg_report[['metadata']][['color_value_box']],
  pcg_report[['metadata']][['color_none']]))
...
flexdashboard::valueBox(n_vus, color = dplyr::if_else(
  n_vus > 0,
  pcg_report[['metadata']][['color_value_box']],
  pcg_report[['metadata']][['color_none']]))
```
...
### Biomarker evidence items

```{r}
flexdashboard::valueBox(n_biomarkers, color = dplyr::if_else(
  n_biomarkers > 0,
  pcg_report[['metadata']][['color_value_box']],
  pcg_report[['metadata']][['color_none']]))
```
...
### CPSR settings - virtual gene panel {data-padding=50}
...
### CPSR - ClinVar variants {data-padding=50}


*  For sample __`r cpsr_rep[['metadata']][['sample_name']]`__, a total of n = __`r NROW(cpsr_clinvar_variants)`__ **germline variants**
are registered with a <i>Pathogenic/Likely pathogenic/VUS</i> clinical significance in ClinVar.
...
### CPSR - non-ClinVar variants {data-padding=50}
...
### CPSR - biomarkers {data-padding=50}

* CPSR detected __n = `r NROW(cpsr_biomarker_eitems)`__ evidence items that overlap **germline variants** in the input sample.
