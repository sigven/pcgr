...
```{r}
flexdashboard::valueBox(length(unique(pcg_report$content$cna$disp$tier1$SEGMENT)), color = pcg_report$settings$conf$visual_reporting$color_value_box, icon = "fa-dna")
```
...
crosstalk::bscols(
  list(
    crosstalk::filter_select(
      ...
      variants_tier1_shared, ~BM_CANCER_TYPE),
    crosstalk::filter_select(
      "BM_CLINICAL_SIGNIFICANCE", "Clinical significance", 
      variants_tier1_shared, ~BM_CLINICAL_SIGNIFICANCE),
    crosstalk::filter_slider(
      "RATING", "Rating", variants_tier1_shared, 
      ~RATING, min = 0, max = 5, step = 1, ticks = T)

  ),
  list(
    crosstalk::filter_select(
      "BM_EVIDENCE_TYPE", "Evidence type", 
      variants_tier1_shared, ~BM_EVIDENCE_TYPE),
    crosstalk::filter_select(
      "BM_EVIDENCE_LEVEL", "Evidence level", 
      variants_tier1_shared, ~BM_EVIDENCE_LEVEL),
    crosstalk::filter_select(
      "BM_THERAPEUTIC_CONTEXT", "Therapeutic context", 
      variants_tier1_shared, ~BM_THERAPEUTIC_CONTEXT)
  )
)
