...
flexdashboard::valueBox(
  ...
  color = ifelse(
    ...
    pcg_report$settings$conf$visual_reporting$color_value_box,
...
drug_levels <- levels(as.factor(
  pcg_report[["content"]][["cna"]][["disp"]][["oncogene_gain"]]$DRUGS_ON_LABEL))
# if(length(drug_levels) > 0){
#   tier3_vars <- tier3_vars |>
#     dplyr::arrange(MUTATION_HOTSPOT, desc(OPENTARGETS_RANK))
# }

oncogene_gain_variants <- crosstalk::SharedData$new(
  dplyr::filter(
    pcg_report[["content"]][["cna"]][["callset"]][["variant"]],
    TIER == 3 & ONCOGENE == TRUE))
crosstalk::bscols(
  list(
    crosstalk::filter_select("SYMBOL", "Gene", oncogene_gain_variants, ~SYMBOL),
    crosstalk::filter_slider("N_MAJOR", "Major copy number", oncogene_gain_variants, ~N_MAJOR),
    crosstalk::filter_slider("N_MINOR", "Minor copy number", oncogene_gain_variants, ~N_MINOR),
  ),
  list(
    crosstalk::filter_select("CHROMOSOME", "Chromosome", oncogene_gain_variants, ~CHROM),
...
dt |>
  DT::formatStyle("SYMBOL","TISSUE_ASSOC_RANK", color = "white", backgroundColor = DT::styleInterval(
    pcg_report$settings$conf$visual_reporting$cancer_assoc$breaks,
    pcg_report$settings$conf$visual_reporting$cancer_assoc$values))
