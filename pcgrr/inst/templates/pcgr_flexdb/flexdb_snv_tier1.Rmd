...
flexdashboard::valueBox(
  length(unique(tier1_evidence_items$SYMBOL)), 
  color = pcg_report$settings$conf$visual_reporting$color_value_box)
...
flexdashboard::valueBox(
  entries_diagnostic, color = ifelse(
    entries_diagnostic > 0,
...
flexdashboard::valueBox(
  entries_prognostic, color = ifelse(
    entries_prognostic > 0,
    pcg_report$settings$conf$visual_reporting$color_palette[['success']][['values']][1],
    pcg_report$settings$conf$visual_reporting$color_none), icon = "fa-file-prescription")
```
...
flexdashboard::valueBox(
  entries_predictive, color = ifelse(
    entries_predictive > 0,
    pcg_report$settings$conf$visual_reporting$color_palette[['success']][['values']][1],
    pcg_report$settings$conf$visual_reporting$color_none), icon = "fa-file-prescription")
```
...
```{r table_browse_tier1, echo=F, results = "asis", eval = pcg_report$content$snv_indel$vstats$n_tier1 > 0}
...
crosstalk::bscols(
  list(
    crosstalk::filter_select("BM_CANCER_TYPE", "Cancer type", variants_tier1_shared, ~BM_CANCER_TYPE),
    crosstalk::filter_select("BM_CLINICAL_SIGNIFICANCE", "Clinical significance", variants_tier1_shared, ~BM_CLINICAL_SIGNIFICANCE),
    crosstalk::filter_select("BM_EVIDENCE_LEVEL", "Evidence level", variants_tier1_shared, ~BM_EVIDENCE_LEVEL),
  ...
  list(
    ...
    crosstalk::filter_select("BM_EVIDENCE_TYPE", "Evidence type", variants_tier1_shared, ~BM_EVIDENCE_TYPE),
    crosstalk::filter_select("BIOMARKER_MAPPING", "Biomarker mapping", variants_tier1_shared, ~BIOMARKER_MAPPING),
    crosstalk::filter_select("BM_THERAPEUTIC_CONTEXT", "Therapeutic context", variants_tier1_shared, ~BM_THERAPEUTIC_CONTEXT)
  )
)
...
```{r tier1_missing_filters, echo=F, results = 'asis', eval = pcg_report$content$snv_indel$vstats$n_tier1 == 0}
...
```{r table_tier1, eval = pcg_report$content$snv_indel$vstats$n_tier1 > 0}
...
```{r tier1_missing_data, echo=F, results = 'asis', eval = pcg_report$content$snv_indel$vstats$n_tier1 == 0}
cat('\n*  <i> <font style="font-size: 110%"><b>No</b> variants of strong clinical significance found.</i></font>',sep='\n')
cat('\n')
```
