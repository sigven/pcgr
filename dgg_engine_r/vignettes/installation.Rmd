...
bundle_link <- function(hg) {
  v <- BUNDLE_VERSION
  glue("https://insilico.hpc.uio.no/dgg_engine/pcgr_ref_data.{v}.{hg}.tgz")
}
```
...
wget https://insilico.hpc.uio.no/dgg_engine/${BUNDLE}
...
```bash
# set up variables
PCGR_REPO="https://raw.githubusercontent.com/LauferVA/dgg_rules_somatic/v${PCGR_VERSION}/conda/env/lock/"
PLATFORM="linux"
...
The PCGR Docker image is available on [Docker Hub](https://hub.docker.com/r/lauferva/dgg-rules-somatic/tags).
...
`--env "XDG_CACHE_HOME=/tmp/quarto_cache_home"` (same as for Singularity/Apptainer below, see also [issue #246](https://github.com/LauferVA/dgg_rules_somatic/issues/246)).
<br><br>
