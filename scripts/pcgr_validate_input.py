...
def __main__():
    ...
    parser.add_argument('refdata_dir',help='Directory with reference data for PCGR/CPSR')
    ...
    ret = validate_pcgr_input(args.refdata_dir,
                              args.input_vcf,
                              args.validated_vcf,
                              args.input_cna,
...
def validate_pcgr_input(refdata_directory,
                        input_vcf,
                        validated_vcf,
                        input_cna,
                        input_rna_fusion,
    ...
    if not input_vcf == 'None':
        ...
        vcf_infotags['pcgr'] = read_infotag_file(
            os.path.join(refdata_directory,'data',genome_assembly, 'vcf_infotags_other.tsv'), scope = "pcgr")
        vcf_infotags['vep'] = read_infotag_file(
            os.path.join(refdata_directory,'data',genome_assembly, 'vcf_infotags_vep.tsv'), scope = "vep")
        ...
        ## Check that no INFO annotation tags in the query VCF coincides with those generated by PCGR
