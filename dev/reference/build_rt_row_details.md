...
Build JavaScript function for reactable row details Generates a JS
function that can be used in reactable's .details argument to
(optionally) render additional details for each row when expanded. The
function excludes specified primary columns, styling columns, and
...
- styling_cols:
  ...
  Character vector of column names used for styling (e.g.
...
.details argument to render row details with the specified exclusions
and formatting.
