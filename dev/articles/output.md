...
| 7\. `TWOHIT_CANDIDATE_SOMATIC` | Comma-separated somatic loss-of-function variants overlapping the segment (TSG with allele-specific LOH: deletion or copy-neutral). Each entry has the format `VAR_ID;CONSEQUENCE;VAF_FLAG`, where `VAF_FLAG` is one of: `VAF_CONSISTENT` (observed VAF consistent with variant on retained allele given tumor purity), `VAF_LOW` (VAF below purity-adjusted expectation — variant may reside on the deleted allele or be subclonal to the LOH event), or `VAF_UNKNOWN` (tumor purity not provided). |
