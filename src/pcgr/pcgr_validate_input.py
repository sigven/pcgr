...
def check_existing_vcf_info_tags(input_vcf, pcgr_directory, logger):
   ...
   vcfanno_tags = {}
   for db in ['intogen_driver_mut','dbsnp','docm','civic','cbmdb','dbnsfp','clinvar','tcga','uniprot','cancer_hotspots','pcgr_onco_xref']:
...
def check_format_ad_dp_tags(vcf, configuration_file, logger):
   ...
   config_options = pcgrutils.read_config_options(configuration_file, logger)
...
def validate_pcgr_input(pcgr_directory, input_vcf, input_cna, configuration_file):
   """
   ...
   if not input_vcf == 'None':
      ...
      vcf = VCF(input_vcf)
      allelic_support_check = check_format_ad_dp_tags(vcf, configuration_file, logger)
