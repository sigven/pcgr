...
predict_msi_status <- function(vcf_data_df, simpleRepeats_gr, windowMasker_gr, msi_prediction_model, indelFracPlot_template, sample_name = 'Test'){
  ...
  mutations_valid <- pcgrr2::get_valid_chromosomes(vcf_data_df, chromosome_column = 'CHROM', bsg = BSgenome.Hsapiens.UCSC.hg19)
  mutations_valid <- dplyr::select(mutations_valid, CHROM,POS,end,REF,ALT,CONSEQUENCE,SYMBOL,GENOMIC_CHANGE,VARIANT_CLASS,PROTEIN_DOMAIN,GENE_NAME,PROTEIN_CHANGE,PROTEIN_FEATURE,CANCER_MUTATION_HOTSPOT,OTHER_DISEASE_DOCM,OTHER_LITERATURE_DOCM,CLINVAR,AF_TUMOR, DP_TUMOR,AF_NORMAL,DP_NORMAL,CALL_CONFIDENCE)
