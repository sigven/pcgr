...
get_cna_cytoband <- function(cna_gr, cytoband_gr){
  ...
  cyto_stats <- pcgrr2::df_string_replace(cyto_stats, c("cytoband"), pattern = ", (\\S+, ){0,}", replacement = " - ")
...
cna_segment_annotation <- function(cna_file, logR_gain, logR_homdel, pcgr_data){
  ...
  cytoband_df <- pcgrr2::get_cna_cytoband(cna_gr, pcgr_data$cytoband_gr)
  ...
  df_print_sorted <- pcgrr2::df_string_replace(df_print_sorted, strings = c("cancer_census_somatic","cancer_census_germline","antineoplastic_drugs_dgidb"),pattern = "&", replacement = ", ")
  ...
  df <- dplyr::rename(df, ONCOGENE = ts_oncogene, TUMOR_SUPPRESSOR = tsgene, ENTREZ_ID = entrezgene, CANCER_CENSUS_SOMATIC = cancer_census_somatic, GENE = symbol, CHROMOSOME = chrom, GENENAME = name, ANTINEOPLASTIC_DRUG_INTERACTIONS = DGIDBLINK, SEGMENT_LENGTH_MB = segment_length_Mb, SEGMENT = segment_link, TRANSCRIPT_OVERLAP = transcript_overlap_percent)
  ...
  df <- dplyr::select(df, CHROMOSOME, GENE, GENE_NAME, CANCER_CENSUS_SOMATIC, KEGG_PATHWAY, TUMOR_SUPPRESSOR, ONCOGENE, ANTINEOPLASTIC_DRUG_INTERACTIONS,SEGMENT_LENGTH_MB, SEGMENT, gencode_transcript_type,LogR, TRANSCRIPT_OVERLAP) %>% dplyr::distinct()
  ...
  oncogene_amplified <- dplyr::filter(df, !is.na(ONCOGENE) & TRANSCRIPT_OVERLAP == 100 & LogR >= logR_gain & gencode_transcript_type == 'protein_coding')
  ...
  tsgene_homozygous_deletion <- dplyr::filter(df, !is.na(TUMOR_SUPPRESSOR) & TRANSCRIPT_OVERLAP == 100  & LogR <= logR_homdel & gencode_transcript_type == 'protein_coding')
  ...
  civic_cna_biomarkers <- dplyr::filter(pcgr_data$civic_biomarkers, alteration_type == 'CNA' & !is.na(civic_consequence)) %>% dplyr::select(genesymbol,evidence_type,evidence_level,evidence_description,cancer_type,evidence_direction,pubmed_html_link,therapeutic_context,rating,clinical_significance,civic_consequence)
  ...
  if(!is.null(cna_biomarkers)){
    if(nrow(cna_biomarkers) > 0){
      cna_biomarkers <- cna_biomarkers[c("GENE","CANCER_TYPE","CNA_TYPE","EVIDENCE_LEVEL","CLINICAL_SIGNIFICANCE","EVIDENCE_TYPE","DESCRIPTION","EVIDENCE_DIRECTION","THERAPEUTIC_CONTEXT","CITATION","RATING","GENE_NAME","CANCER_CENSUS_SOMATIC","KEGG_PATHWAY","ANTINEOPLASTIC_DRUG_INTERACTIONS","SEGMENT_LENGTH_MB", "SEGMENT","LogR")]
