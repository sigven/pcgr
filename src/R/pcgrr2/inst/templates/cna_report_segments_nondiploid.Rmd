...
* <span style="color:#009E73"><b>Copy number amplifications</b></span>: Log(2) ratio >= `r logR_gain`
* <span style="color:red"><b>Homozygous deletions</b></span>: Log(2) ratio <= `r logR_homdel`

A total of __`r nrow(cna_data$ranked_segments)`__ unfiltered aberration segments satisfied the above criteria.
...
if(nrow(cna_data$ranked_segments) > 0){
  cna_data$ranked_segments <- dplyr::rename(cna_data$ranked_segments, Segment = SEGMENT, Length_Mb = SEGMENT_LENGTH_MB, Cytoband = cytoband, Event_type = event_type)
  cna_segments_sd <- crosstalk::SharedData$new(cna_data$ranked_segments)
}else{
  cna_segments_sd <- crosstalk::SharedData$new(cna_data$ranked_segments)
}
...
crosstalk::bscols(
  ...
  DT::formatStyle('LogR',color='white', backgroundColor = DT::styleInterval(logR_homdel, c('red', '#009E73')))
)
