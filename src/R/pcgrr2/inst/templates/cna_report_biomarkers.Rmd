...
A total of __`r nrow(cna_data$cna_biomarker_segments)`__ aberrations are associated with clinical evidence items in the [database for clinical interpretations of variants in cancer, CIViC](https://civic.genome.wustl.edu/#/home), with the following number of evidence items:

 *  Predictive: __`r nrow(cna_data$cna_evidence_items_predictive)`__ evidence items linked to drug sensitivity/resistance
 *  Prognostic: __`r nrow(cna_data$cna_evidence_items_prognostic)`__ evidence items
 *  Diagnostic: __`r nrow(cna_data$cna_evidence_items_diagnostic)`__ evidence items
 *  Predisposing: __`r nrow(cna_data$cna_evidence_items_predisposing)`__ evidence items
...
if(nrow(cna_data$cna_evidence_items_diagnostic) > 0){
  show_cna_diagnostic_filters <- T
  missing_cna_diagnostic_variants <- F
  cna_data$cna_evidence_items_diagnostic <- dplyr::rename(cna_data$cna_evidence_items_diagnostic, Segment = SEGMENT, Length_Mb = SEGMENT_LENGTH_MB)
}

show_cna_prognostic_filters <- F
missing_cna_prognostic_variants <- T
if(nrow(cna_data$cna_evidence_items_prognostic) > 0){
  show_cna_prognostic_filters <- T
  missing_cna_prognostic_variants <- F
  cna_data$cna_evidence_items_prognostic <- dplyr::rename(cna_data$cna_evidence_items_prognostic, Segment = SEGMENT, Length_Mb = SEGMENT_LENGTH_MB)
}

show_cna_predictive_filters <- F
missing_cna_predictive_variants <- T
if(nrow(cna_data$cna_evidence_items_predictive) > 0){
  show_cna_predictive_filters <- T
  missing_cna_predictive_variants <- F
  cna_data$cna_evidence_items_predictive <- dplyr::rename(cna_data$cna_evidence_items_predictive, Segment = SEGMENT, Length_Mb = SEGMENT_LENGTH_MB)
}

show_cna_predisposing_filters <- F
missing_cna_predisposing_variants <- T
if(nrow(cna_data$cna_evidence_items_predisposing) > 0){
  show_cna_predisposing_filters <- T
  missing_cna_predisposing_variants <- F
  cna_data$cna_evidence_items_predisposing <- dplyr::rename(cna_data$cna_evidence_items_predisposing, Segment = SEGMENT, Length_Mb = SEGMENT_LENGTH_MB)
}
...
cna_predictive_shared <- crosstalk::SharedData$new(cna_data$cna_evidence_items_predictive)
crosstalk::bscols(
  list(
...
cna_prognostic_shared <- crosstalk::SharedData$new(cna_data$cna_evidence_items_prognostic)
crosstalk::bscols(
  list(
...
cna_diagnostic_shared <- crosstalk::SharedData$new(cna_data$cna_evidence_items_diagnostic)
crosstalk::bscols(
  list(
...
cna_predisposing_shared <- crosstalk::SharedData$new(cna_data$cna_evidence_items_predisposing)
crosstalk::bscols(
  list(
