...
#'
#'
signature_contributions_single_sample <- function(sample_calls,
                                                  sample_name,
  ...
  assertable::assert_colnames(sample_calls, c("VCF_SAMPLE_ID", "CHROM", "POS", "REF", "ALT"),
                              only_colnames = F, quiet = T)
  sample_calls <- pcgrr::get_valid_chromosomes(sample_calls)
  sigs.input <- deconstructSigs::mut.to.sigs.input(mut.ref = sample_calls,
  ...
  while (n <= ncol(nonzero_signatures)) {
    df <- data.frame("sample_name" = sample_name,
                     "signature_id" = stringr::str_replace(colnames(nonzero_signatures)[n], "ignature\\.", ""),
  ...
  signature_contributions <- rbind(signature_contributions, data.frame("sample_name" = sample_name,
                                                                       "signature_id" = "unknown",
                                                                       "weight" = sample_1$unknown))
  signature_rows <- as.numeric(stringr::str_replace(as.character(signature_contributions[signature_contributions$signature_id != "unknown", ]$signature_id), "S", ""))
  weight_df <- data.frame("Signature_ID" = as.character(signature_contributions$signature_id),
                          "Weight" = round(as.numeric(signature_contributions$weight), digits = 3),
                          stringsAsFactors = F)

  signatures_cancertypes_aetiologies <- cosmic_signatures_aetiologies[signature_rows, ] %>%
    dplyr::left_join(weight_df, by = c("Signature_ID")) %>%
    dplyr::arrange(desc(Weight)) %>%
    dplyr::select(Signature_ID, Weight, Cancer_types, Proposed_aetiology, Comments, Keyword) %>%
    dplyr::mutate(Trimer_normalization_method = normalization_method, Sample_Name = sample_name)
  ...
  if (nrow(signature_contributions[signature_contributions$signature_id == "unknown", ]) > 0) {
    unknown_df <- as.data.frame(
      signature_contributions %>%
        ...
        dplyr::mutate(Weight = round(as.numeric(Weight), digits = weight_precision)) %>%
        dplyr::mutate(Sample_Name = as.character(sample_name), Trimer_normalization_method = as.character(normalization_method)) %>%
...
#
#   pcg_report_signatures <- pcgrr::init_report(pcgr_config, sample_name, class = "m_signature")
#
#   exonic <- FALSE
#   if (pcgr_config$mutational_signatures$mutsignatures_normalization == "exome2genome" |
#       pcgr_config$mutational_signatures$mutsignatures_normalization == "exome") {
#     exonic <- TRUE
#   }
#
#   rlogging::message("------")
...
#         dplyr::select(CHROM, POS, REF, ALT, VCF_SAMPLE_ID)
#       pcg_report_signatures[["eval"]] <- TRUE
#       mut_signature_contributions <-
#         pcgrr::signature_contributions_single_sample(pcg_report_signatures[["variant_set"]][["all"]],
...
#       }
#       rlogging::message(paste0("Too few SNVs (n = ", nrow(pcg_report_signatures[["variant_set"]][["all"]]),
#                                ") for reconstruction of mutational signatures by deconstructSigs, limit set to ",
#                                pcgr_config[["mutational_signatures"]][["mutsignatures_mutation_limit"]]))
...
#' Function that generates mutational signatures data for PCGR report
#'
#' @param vcf_fname VCF file processed with PCGR annotation pipeline - possibly filtered for depth/allelic fraction
#' @param pcgr_data object with PCGR annotation data
#' @param sample_name sample identifier
#' @param pcgr_config Object with PCGR configuration parameters
#' @param type_specific logical indicating if all reference signatures are to be included (F)
#' rather than those known to be prevalent in the tumor (T)
#'
generate_report_data_signatures_mp <- function(vcf_fname, pcgr_data, sample_name,
                                               pcgr_config,
                                               type_specific = T) {

  rlogging::message("------")
  rlogging::message("Identifying weighted contributions of reference mutational signatures (COSMIC v3) using MutationalPatterns")
  assay <- tolower(pcgr_config$assay_properties$type)
  pcg_report_signatures <- pcgrr::init_report(pcgr_config, sample_name, class = "m_signature_mp")
  reference_data <-
    pcgrr::get_reference_signature_data(pcgr_config[["tumor_properties"]][["tumor_type"]],
                                        assay = assay)
  if(type_specific == F){
    reference_data <- pcgrr::get_reference_signature_data("Any", assay = assay)
  }
  if(file.exists(vcf_fname)){
    vcfs <- suppressWarnings(MutationalPatterns::read_vcfs_as_granges(vcf_fname, sample_name,
                                                                    pcgr_data[["assembly"]][["ref_genome"]]))

    rlogging::message(paste0("Number of SNVs for signature analysis: ", length(vcfs[[1]])))

    pcg_report_signatures[["eval"]] <- TRUE

    if (length(vcfs[[1]]) >= pcgr_config[["msigs"]][["mutation_limit"]]) {
      pcg_report_signatures[["variant_set"]][["all"]] <- as.data.frame(vcfs[[1]]) %>%
        dplyr::rename(POS = start, CHROM = seqnames) %>%
        dplyr::select(CHROM, POS, REF, ALT) %>%
        magrittr::set_rownames(NULL)

      ## get context matrix
      mut_mat <- MutationalPatterns::mut_matrix(vcf_list = vcfs, ref_genome = pcgr_data[["assembly"]][["ref_genome"]])
      mut_mat <- mut_mat + 0.0001

      ## reconstruct mutation profile from reference mutational signatures
      fit_ref <- MutationalPatterns::fit_to_signatures(mut_mat, reference_data$matrix)
      ...
      sim_original_reconstructed <-
        as.data.frame(MutationalPatterns::cos_sim_matrix(mut_mat, fit_ref[["reconstructed"]])) %>%
        magrittr::set_colnames("cosine_sim") %>%
        magrittr::set_rownames(NULL)

      ## assess the relative contribution of each reference mutational signature
      tot <- as.data.frame(setNames(reshape2::melt(colSums(fit_ref[["contribution"]])), c("tot"))) %>%
        dplyr::mutate(sample_id = as.character(rownames(.))) %>%
        magrittr::set_rownames(NULL)

      ## add information on aetiologies, and aggregate contributions pr. aetiology
      contributions_per_signature <-
        as.data.frame(setNames(reshape2::melt(fit_ref[["contribution"]]),
                               c("signature_id", "sample_id", "contribution_raw"))) %>%
        dplyr::mutate(signature_id = as.character(signature_id)) %>%
        dplyr::mutate(sample_id = as.character(sample_id)) %>%
        dplyr::left_join(tot, by = "sample_id") %>%
        dplyr::mutate(prop_signature = round(as.numeric(contribution_raw) / tot, digits = 3)) %>%
        dplyr::select(signature_id, sample_id, prop_signature) %>%
        dplyr::filter(prop_signature > 0) %>%
        dplyr::arrange(desc(prop_signature)) %>%
        dplyr::left_join(dplyr::select(pcgr_data[["mutational_signatures"]][["aetiologies_67"]], signature_id,
                                       aetiology, comments, aetiology_keyword), by = c("signature_id")) %>%
        dplyr::rename(group = aetiology_keyword) %>%
        dplyr::mutate(contribution = paste0(round(prop_signature * 100, digits = 2), "%")) %>%
        dplyr::distinct()
      ...
      if(!is.null(reference_data$aetiology) & NROW(contributions[['per_signature']]) > 0){
        if("signature_id" %in% colnames(reference_data$aetiology)){
          reference_sigs <- paste(sort(reference_data$aetiology$signature_id), collapse=",")
          tsv_data <- contributions[['per_signature']] %>%
            pcgrr::remove_cols_from_df(cnames = c("contribution",'col','aetiology','comments')) %>%
            dplyr::mutate(all_reference_signatures = !type_specific,
                          ...
                          fitting_accuracy = round(sim_original_reconstructed$cosine_sim * 100, digits = 1))
        }
      }
      ...
      pcg_report_signatures[["result"]][["scale_fill_names"]] <- names(color_vec)
      pcg_report_signatures[["result"]][["goodness_of_fit"]] <- round(sim_original_reconstructed$cosine_sim * 100, digits = 1)
    }else{
      pcg_report_signatures[["missing_data"]] <- TRUE
      if (length(vcfs[[1]]) > 0) {
        pcg_report_signatures[["variant_set"]][["all"]] <- as.data.frame(vcfs[[1]]) %>%
          ...
          magrittr::set_rownames(NULL)
        rlogging::message(paste0("Too few SNVs (n = ", nrow(pcg_report_signatures[["variant_set"]][["all"]]),
                                 ") for reconstruction of mutational signatures by MutationalPatterns, limit set to ",
...
get_reference_signature_data <- function(site, assay = "wes", prevalence_pct = 2,
                                         ignore_seq_artefacts = T) {

  rlogging::message("Retrieving prevalent (prevalence >= ", prevalence_pct, " percent) reference signatures for ", site, ", using COSMIC v3 collection (", toupper(assay), ")")
  #assertthat::assert_that(!)
  invisible(assertthat::assert_that(!is.null(pcgr_data[["mutational_signatures"]][["aetiologies_67"]]), msg = "Cannot load reference collection (COSMIC v3) of mutational signatures"))
  invisible(assertthat::assert_that(is.data.frame(pcgr_data[["mutational_signatures"]][["aetiologies_67"]]), msg = "Reference collection must be of type data.frame()"))
  invisible(assertthat::assert_that(assay == "wes" | assay == "wgs", msg = "Argument 'assay' must be either 'wes' or 'wgs'"))
  invisible(assertthat::assert_that(prevalence_pct == 2 | prevalence_pct == 5 |
                                    prevalence_pct == 10 | prevalence_pct == 15 |
                                    prevalence_pct == 20, msg = "Argument 'prevalence_pct' must be any of '2, 5, 10, 15 or 20'"))

  ##probabilities of mutation contexts in each of 67 cancer signatures (from ICGC)
  ...
  if (!(site %in% unique_sites_with_signature_prevalence)) {
    rlogging::message(paste0("Primary tumor site '", site, "' does not have any signatures with significant prevalence - considering all"))
    signatures_prevalence <- pcgr_data[["mutational_signatures"]][["aetiologies_67"]] %>%
    ...
    signatures_prevalence <- pcgr_data[["mutational_signatures"]][["aetiologies_67"]] %>%
      ...
      dplyr::select(signature_id, primary_site, prevalence_pct, prevalence_above_5pct,
  ...
  signatures_prevalence <- signatures_prevalence %>%
    dplyr::distinct()
  rlogging::message(paste0("Limiting reference collection to signatures: ", paste(sigs, collapse = ", ")))
  cancer_sigs <- dplyr::select(cancer_sigs, dplyr::one_of(sigs))


  result <- list('matrix' = as.matrix(cancer_sigs), 'aetiology' = signatures_prevalence)
  #cancer_sigs <- as.matrix(cancer_sigs)
...
#' @param colors character vector of six color codes (denoting the different mutation types)
#' @param autosomes logical indicating if plotting should only consider autosomes
#' @param build genome assembly (grch37/grch38)
#'
generate_report_data_rainfall <- function(variant_set, colors = NULL, autosomes = F, build = "grch37") {

  invisible(assertthat::assert_that(assertthat::is.flag(autosomes), msg = "Argument 'autosomes' should be TRUE/FALSE"))
  invisible(assertthat::assert_that(is.data.frame(variant_set), msg = paste0("Argument variant_set needs be of type data.frame")))
  assertable::assert_colnames(variant_set, c("CHROM", "REF", "ALT", "POS", "VARIANT_CLASS"), only_colnames = F, quiet = T)
  invisible(assertthat::assert_that(build == "grch37" | build == "grch38",
                                    msg = paste0("Value for argument build ('", build, "') not allowed, available reference build values are: 'grch37' or 'grch38'")))
  ...
  if (is.null(colors)) {
    ...
    invisible(assertthat::assert_that(length(colors) == 6,
  ...
  dat <- dplyr::select(variant_set, CHROM, POS, REF, ALT, VARIANT_CLASS) %>%
    dplyr::filter(VARIANT_CLASS == "SNV")

  if (nrow(dat) < 10 | length(chromosome_names) < 2) {
    rlogging::message("Too few variants (< 10) and chromosomes represented (< 2) - skipping rainfall plot")
    pcg_report_rainfall[["eval"]] <- F
  }else{
    dat <- dat %>%
      pcgrr::assign_mutation_type() %>%
      dplyr::mutate(MUTATION_TYPE = dplyr::if_else(stringr::str_detect(MUTATION_TYPE, "^C>"),
                                                   stringr::str_replace(MUTATION_TYPE, ":[A-Z]>[A-Z]$", ""),
                                                   as.character(MUTATION_TYPE))) %>%
      dplyr::mutate(MUTATION_TYPE = dplyr::if_else(stringr::str_detect(MUTATION_TYPE, "^A>"),
                                                   stringr::str_replace(MUTATION_TYPE, "^[A-Z]>[A-Z]:", ""),
                                                   as.character(MUTATION_TYPE))) %>%
    ...
    invisible(assertthat::assert_that(length(type) == length(loc) &
                            ...
                            length(chrom) == length(dist), msg = "Length of type/loc/dist not identical"))
    ...
    pcg_report_rainfall[["rfdata"]] <- list("data" = data, "intercept" = m, "ylim" = ylim,
              "chr_cum" = chr_cum, "colors" = colors_selected, "labels" = labels, "cex" = 0.8, "cex_text" = 3)
  }
