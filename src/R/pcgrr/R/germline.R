...
generate_report_data_tumor_only <- function(unfiltered_sample_calls, sample_name, pcgr_config) {
  ...
  ## initiate report
  pcg_report_to <- pcgrr::init_report(pcgr_config, sample_name, class = "tumor_only")

  ## assign evidence tags for germline/somatic state of variants, partially based on user-defined criteria (population allele frequency thresholds)
  ...
  germline_filter_statistics[["remain_post_onekg"]] <-
    nrow(sample_calls) - nrow(sample_calls[sample_calls$SOMATIC_CLASSIFICATION == "GERMLINE_1KG", ])
  ...
  germline_filter_statistics[["remain_post_gnomad"]] <-
    germline_filter_statistics[["remain_post_onekg"]] - nrow(sample_calls[sample_calls$SOMATIC_CLASSIFICATION == "GERMLINE_GNOMAD", ])
  ...
  germline_filter_statistics[["remain_post_clinvar"]] <-
    germline_filter_statistics[["remain_post_gnomad"]] - nrow(sample_calls[sample_calls$SOMATIC_CLASSIFICATION == "GERMLINE_CLINVAR", ])
  ...
  if (pcgr_config[["tumor_only"]][["exclude_pon"]] == TRUE) {
    germline_filter_statistics[["remain_post_pon"]] <-
      germline_filter_statistics[["remain_post_pon"]] - nrow(sample_calls[sample_calls$SOMATIC_CLASSIFICATION == "GERMLINE_PON", ])
  ...
  if (pcgr_config[["tumor_only"]][["exclude_likely_hom_germline"]] == TRUE) {
    germline_filter_statistics[["remain_post_hom"]] <-
      germline_filter_statistics[["remain_post_hom"]] - nrow(sample_calls[sample_calls$SOMATIC_CLASSIFICATION == "GERMLINE_HOMOZYGOUS", ])
  ...
  if (pcgr_config[["tumor_only"]][["exclude_likely_het_germline"]] == TRUE) {
    germline_filter_statistics[["remain_post_het"]] <-
      germline_filter_statistics[["remain_post_het"]] - nrow(sample_calls[sample_calls$SOMATIC_CLASSIFICATION == "GERMLINE_HETEROZYGOUS", ])
  ...
  if (pcgr_config[["tumor_only"]][["exclude_dbsnp_nonsomatic"]] == TRUE) {
    ...
    germline_filter_statistics[["remain_post_dbsnp"]] <-
      germline_filter_statistics[["remain_post_dbsnp"]] - nrow(sample_calls[sample_calls$SOMATIC_CLASSIFICATION == "GERMLINE_DBSNP", ])
  ...
  for (db_filter in c("onekg", "gnomad", "dbsnp", "pon", "clinvar", "hom", "het", "nonexonic")) {
    if (pcg_report_to[["variant_statistic"]][[paste0(db_filter, "_n_remain")]] > 0 & pcg_report_to[["variant_statistic"]][["unfiltered_n"]] > 0) {
      pcg_report_to[["variant_statistic"]][[paste0(db_filter, "_frac_remain")]] <-
        round((as.numeric(pcg_report_to[["variant_statistic"]][[paste0(db_filter, "_n_remain")]]) /
...
assign_somatic_classification <- function(sample_calls, config) {
  ...
  sample_calls <- sample_calls %>%
    ...
    dplyr::mutate(SOMATIC_CLASSIFICATION = dplyr::if_else(STATUS_LIKELY_GERMLINE_HOMOZYGOUS == TRUE &
    ...
    dplyr::mutate(SOMATIC_CLASSIFICATION = dplyr::if_else(STATUS_LIKELY_GERMLINE_HETEROZYGOUS == TRUE &
  ...
  if (config[["tumor_only"]][["exclude_dbsnp_nonsomatic"]] == TRUE) {
    ...
    sample_calls <- sample_calls %>%
      dplyr::mutate(SOMATIC_CLASSIFICATION = dplyr::if_else(STATUS_DBSNP_GERMLINE == TRUE &
...
assign_somatic_germline_evidence <- function(sample_calls, config) {
  ...
  for (pop in c("EUR", "AMR", "AFR", "SAS", "EAS", "GLOBAL")) {
    sample_calls <-
      pcgrr::assign_germline_popfreq_status(sample_calls,
                                            pop = pop, dbquery = "1KG",
                                            max_tolerated_af = config[["tumor_only"]][[paste0("maf_onekg_", tolower(pop))]])
  }
  ...
  for (pop in c("GLOBAL", "NFE", "AMR", "AFR", "SAS", "EAS", "ASJ", "FIN", "OTH")) {
    sample_calls <-
      pcgrr::assign_germline_popfreq_status(sample_calls,
                                            pop = pop,
                                            dbquery = "gnomAD",
                                            max_tolerated_af = config[["tumor_only"]][[paste0("maf_gnomad_", tolower(pop))]])
  }
  ...
  for(c in onekg_cols){
    if(nrow(sample_calls[!is.na(sample_calls[,c]) & sample_calls[,c] > sample_calls$MAX_AF_1KG,]) > 0){
      sample_calls[!is.na(sample_calls[,c]) & sample_calls[,c] > sample_calls$MAX_AF_1KG,"MAX_AF_1KG"] <-
        sample_calls[!is.na(sample_calls[,c]) & sample_calls[,c] > sample_calls$MAX_AF_1KG,c]
    }
  ...
  for(c in gnomad_cols){
    if(nrow(sample_calls[!is.na(sample_calls[,c]) & sample_calls[,c] > sample_calls$MAX_AF_GNOMAD,]) > 0){
      sample_calls[!is.na(sample_calls[,c]) & sample_calls[,c] > sample_calls$MAX_AF_GNOMAD,"MAX_AF_GNOMAD"] <-
        sample_calls[!is.na(sample_calls[,c]) & sample_calls[,c] > sample_calls$MAX_AF_GNOMAD,c]
    }
  ...
  if ("DBSNPRSID" %in% colnames(sample_calls)) {
    sample_calls <- sample_calls %>%
      ...
      dplyr::mutate(STATUS_DBSNP_GERMLINE = dplyr::if_else(STATUS_DBSNP_GERMLINE == T & !is.na(DOCM_PMID),
                                                           FALSE, STATUS_DBSNP_GERMLINE)) %>%
      dplyr::mutate(STATUS_DBSNP_GERMLINE = dplyr::if_else(STATUS_DBSNP_GERMLINE == T & !is.na(CLINVAR_MSID) &
  ...
  if ("CLINVAR_MSID" %in% colnames(sample_calls)) {
    sample_calls <- sample_calls %>%
      dplyr::mutate(STATUS_CLINVAR_GERMLINE = dplyr::if_else(!is.na(CLINVAR_MSID) &
  ...
  ## assign STATUS_LIKELY_GERMLINE_HETEROZYGOUS to all calls that have the alternative allele
  ...
  if ("AF_TUMOR" %in% colnames(sample_calls) & "MAX_AF_GNOMAD" %in% colnames(sample_calls) &
      "STATUS_COSMIC" %in% colnames(sample_calls) & "STATUS_TCGA_SOMATIC" %in% colnames(sample_calls)) {
    sample_calls <- sample_calls %>%
      dplyr::mutate(STATUS_LIKELY_GERMLINE_HETEROZYGOUS = dplyr::if_else(!is.na(MAX_AF_GNOMAD) &
...
#' @return sample_calls
#'
assign_germline_popfreq_status <- function(sample_calls, pop = "EUR", dbquery = "1KG",
  ...
  if (dbquery == "1KG") {
    ...
    if (any(grepl(paste0("^", col, "$"), names(sample_calls)))) {
      ...
      if(nrow(sample_calls[!is.na(sample_calls[,col]) &
...
assign_poplevel_frequency_class <- function(sample_calls, pop_af_column) {
  ...
  if (any(grepl(paste0("^", pop_af_column, "$"), names(sample_calls)))) {
    sample_calls <- sample_calls %>%
      dplyr::mutate(pop_common =
                      dplyr::if_else(!is.na(!!rlang::sym(pop_af_column)) &
                                       !!rlang::sym(pop_af_column) >= 0.05,
                                     "Common", as.character(NA))) %>%
      dplyr::mutate(pop_lowfreq =
                      dplyr::if_else(!is.na(!!rlang::sym(pop_af_column)) &
                                       !!rlang::sym(pop_af_column) >= 0.01 &
                                       !!rlang::sym(pop_af_column) < 0.05,
                                     "LowFreq", as.character(NA))) %>%
      dplyr::mutate(pop_rare =
                      dplyr::if_else(!is.na(!!rlang::sym(pop_af_column)) &
                                       !!rlang::sym(pop_af_column) >= 0.001 &
                                       !!rlang::sym(pop_af_column) < 0.01,
                                     "Rare", as.character(NA))) %>%
      dplyr::mutate(pop_veryrare = dplyr::if_else(!is.na(!!rlang::sym(pop_af_column)) &
                                                    !!rlang::sym(pop_af_column) > 0 &
                                                    !!rlang::sym(pop_af_column) < 0.001,
                                                  "VeryRare", as.character(NA))) %>%
      dplyr::mutate(pop_monomorphic = dplyr::if_else(!is.na(!!rlang::sym(pop_af_column)) &
                                                       !!rlang::sym(pop_af_column) == 0,
                                                     "Monomorphic", as.character(NA)))
  }
  return(sample_calls)
...
#' @return sample_calls
#'
assign_poplevel_frequency <- function(sample_calls, dbquery = "1KG",
                                      pop = "european", result_tag = "FREQ_GNOMAD_EUROPEAN") {
  ...
  #sample_calls <- sample_calls %>%
    #dplyr::mutate()
  sample_calls[result_tag] <- stringr::str_replace_all(stringr::str_replace_all(paste(sample_calls$pop_monomorphic,
                                                                                      sample_calls$pop_rare,
                                                                                      sample_calls$pop_veryrare,
                                                                                      sample_calls$pop_lowfreq,
                                                                                      sample_calls$pop_common, sep = ","),
                                                                                "(,{0,}NA(,) {0,}) {1,}", ","), "(^,)|(,$)", "")
...
make_upset_plot_data <- function(calls, config) {
  ...
  assertable::assert_colnames(calls, c("VAR_ID", "STATUS_POPFREQ_1KG_ABOVE_TOLERATED",
                                       "STATUS_POPFREQ_GNOMAD_ABOVE_TOLERATED", "STATUS_CLINVAR_GERMLINE"),
...
upset_plot_tumor_only <- function(upset_data) {
  ...
  p <- UpSetR::upset(upset_data, sets = isets, sets.bar.color = "#56B4E9",
                     order.by = "freq", nintersects = 20, text.scale = 1.5, show.numbers = T,
                     point.size = 6, color.pal = "Blues", empty.intersections = "on")
  return(p)
