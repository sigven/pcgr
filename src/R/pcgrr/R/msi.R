...
#'
#'
predict_msi_status <- function(vcf_data_df, pcgr_data, msi_prediction_model,
                               msi_prediction_dataset, target_size_mb, sample_name = "Test") {
  ...
  mmr_pol_df <- mutations_valid %>%
    dplyr::filter(stringr::str_detect(SYMBOL, "^(MLH1|MLH3|MSH2|MSH3|MSH6|PMS1|PMS2|POLD1|POLE)$") &
                    stringr::str_detect(CONSEQUENCE, "frameshift_|missense_|splice_|stop_|inframe_"))
  mmr_pol_df <- dplyr::select(mmr_pol_df, -c(CHROM, POS, REF, ALT))
  mmr_pol_df <- dplyr::rename(mmr_pol_df, GENE = SYMBOL)
  mmr_pol_df <- mmr_pol_df %>% dplyr::select(GENE, CONSEQUENCE, PROTEIN_CHANGE,
                                             GENE_NAME, VARIANT_CLASS, PROTEIN_DOMAIN,
                                             dplyr::everything())

  msi_predictors <- c("fracWinMaskIndels", "fracWinMaskSNVs", "fracRepeatIndels",
                      "fracNonRepeatIndels", "fracIndels", "MLH1", "MLH3", "MSH2",
                      "MSH3", "MSH6", "PMS1", "PMS2", "POLD1", "POLE", "tmb",
                      "tmb_indel", "tmb_snv")
...
generate_report_data_msi <- function(sample_calls, pcgr_data, sample_name, pcgr_config) {
  ...
  rlogging::message(paste0("n = ", nrow(msi_sample_calls), " exonic variants used for MSI prediction"))
  if (nrow(msi_sample_calls) >= 1) {
    pcg_report_msi[["prediction"]] <- pcgrr::predict_msi_status(msi_sample_calls, pcgr_data,
                                                                ...
                                                                target_size_mb = pcgr_config$assay_properties$target_size_mb,
