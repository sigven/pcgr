...
assign_tier1_tier2_acmg <- function(pcg_report_snv_indel) {
  ...
  ## TIER 1: evidence items in specific tumor type and f high clinical evidence (A_B)
  ## TIER 2: evidence items in other tumor types of high clinical evidence (A_B) and low clinical evidence in
  ## specific tumor type
  ...
  ## eitems
  eitems_specific_ttype <- pcg_report_snv_indel[["clin_eitem"]][["specific_ttype"]]
  eitems_any_ttype <- pcg_report_snv_indel[["clin_eitem"]][["any_ttype"]]
  eitems_other_ttype <- pcg_report_snv_indel[["clin_eitem"]][["other_ttype"]]


  for (etype in c("diagnostic", "predictive", "prognostic")) {
    if (nrow(eitems_specific_ttype[[etype]][["A_B"]]) > 0) {
      vars <- dplyr::select(eitems_specific_ttype[[etype]][["A_B"]], GENOMIC_CHANGE) %>%
        dplyr::distinct()
  ...
  for (etype in c("diagnostic", "predictive", "prognostic")) {
    if (nrow(eitems_any_ttype[[etype]][["A_B"]]) > 0) {
      ...
      if (nrow(eitems_specific_ttype[[etype]][["A_B"]]) > 0) {
        ...
        if(pcgrr::check_common_colnames(df1 = eitems_any_ttype[[etype]][["A_B"]],
      ...
      if (nrow(eitems_other_ttype[[etype]][["A_B"]]) > 0) {
        if (nrow(unique_variants_tier1) > 0) {
          if(pcgrr::check_common_colnames(df1 = unique_variants_tier1,
        ...
        if (nrow(eitems_other_ttype[[etype]][["A_B"]]) > 0) {
          unique_variants_tier2 <- unique_variants_tier2 %>%
            dplyr::bind_rows(dplyr::select(eitems_other_ttype[[etype]][["A_B"]], GENOMIC_CHANGE)) %>%
            dplyr::distinct()
        }
      }
    }
      ...
      if (nrow(unique_variants_tier1) > 0) {
        if(pcgrr::check_common_colnames(df1 = unique_variants_tier1,
          ...
          eitems_specific_ttype[[etype]][["C_D_E"]] <-
            dplyr::anti_join(eitems_specific_ttype[[etype]][["C_D_E"]],
                             unique_variants_tier1, by = c("GENOMIC_CHANGE"))
        }
      }
      if (nrow(eitems_specific_ttype[[etype]][["C_D_E"]]) > 0) {
        unique_variants_tier2 <- unique_variants_tier2 %>%
          dplyr::bind_rows(dplyr::select(eitems_specific_ttype[[etype]][["C_D_E"]], GENOMIC_CHANGE)) %>%
          dplyr::distinct()
      }
    }
  }
  ...
  pcg_report_snv_indel[["clin_eitem"]][["specific_ttype"]] <- eitems_specific_ttype
  pcg_report_snv_indel[["clin_eitem"]][["any_ttype"]] <- eitems_any_ttype
  pcg_report_snv_indel[["clin_eitem"]][["other_ttype"]] <- eitems_other_ttype

  if (nrow(unique_variants_tier1) > 0) {
    if(pcgrr::check_common_colnames(df1 = pcg_report_snv_indel[["variant_set"]][["tier1"]],
    ...
    if(pcgrr::check_common_colnames(df1 = pcg_report_snv_indel[["variant_set"]][["tier2"]],
                                    df2 = unique_variants_tier1,
                                    cnames = c("GENOMIC_CHANGE"))){
...
assign_tier1_tier2_acmg_cna <- function(pcg_report_cna) {
  ...
  for (etype in c("diagnostic", "predictive", "prognostic")) {
    if (nrow(eitems_specific_ttype[[etype]][["A_B"]]) > 0) {
      vars <- dplyr::select(eitems_specific_ttype[[etype]][["A_B"]], SYMBOL, SEGMENT, CNA_TYPE) %>%
        dplyr::distinct()
      unique_variants_tier1 <- rbind(unique_variants_tier1, vars) %>% dplyr::distinct()
    }
  }
  ...
  for (etype in c("diagnostic", "predictive", "prognostic")) {
      ...
      if (nrow(eitems_other_ttype[[etype]][["A_B"]]) > 0) {
        if (nrow(unique_variants_tier1) > 0) {
          if(pcgrr::check_common_colnames(df1 = unique_variants_tier1,
            ...
            eitems_other_ttype[[etype]][["A_B"]] <-
              dplyr::anti_join(eitems_other_ttype[[etype]][["A_B"]],
                               unique_variants_tier1, by = c("SYMBOL","SEGMENT","CNA_TYPE"))
          }
        }
        if (nrow(eitems_other_ttype[[etype]][["A_B"]]) > 0) {
          unique_variants_tier2 <- unique_variants_tier2 %>%
            dplyr::bind_rows(dplyr::select(eitems_other_ttype[[etype]][["A_B"]], SYMBOL, SEGMENT, CNA_TYPE)) %>%
            dplyr::distinct()
        }
      }
    }
    if (nrow(eitems_specific_ttype[[etype]][["C_D_E"]]) > 0) {
      ...
      if (nrow(eitems_specific_ttype[[etype]][["C_D_E"]]) > 0) {
        unique_variants_tier2 <- unique_variants_tier2 %>%
          dplyr::bind_rows(dplyr::select(eitems_specific_ttype[[etype]][["C_D_E"]], SYMBOL, SEGMENT, CNA_TYPE)) %>%
          dplyr::distinct()
      }
    }
  }

  pcg_report_cna[["variant_display"]][["tier1"]] <- unique_variants_tier1
  pcg_report_cna[["variant_display"]][["tier2"]] <- unique_variants_tier2
