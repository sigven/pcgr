...
generate_tier_tsv <- function(variant_set, pcgr_data, pcgr_config, sample_name = 'test'){
  ...
  for(tier in c('tier1','tier2','tier3','tier4','noncoding')){
    if(nrow(variant_set[[tier]]) > 0){
      ...
      tierset <- tierset %>% dplyr::select(dplyr::one_of(pcgr_data$pcgr_tsv_tiered_columns)) %>% dplyr::distinct()

      tsv_variants <- rbind(tsv_variants, tierset)
    }
  }
...
#' @return vcf_data_df
#'
#get_calls <- function(vcf_gz_file, pcgr_data, pcgr_version, sample_name, pcgr_config, genome_seq = BSgenome.Hsapiens.UCSC.hg19, genome_assembly = 'hg19'){
get_calls <- function(tsv_gz_file, pcgr_data, pcgr_version, sample_name, pcgr_config, genome_seq = BSgenome.Hsapiens.UCSC.hg19, genome_assembly = 'hg19'){
  ...
  pcgr_columns <- c('GENOME_VERSION','PROTEIN_CHANGE','CONSEQUENCE','CODING_STATUS','GENOMIC_CHANGE','VAR_ID','DOCM_DISEASE','DOCM_LITERATURE','CLINVAR','CLINVAR_TRAITS_ALL','GENE_NAME','GENENAME','TARGETED_DRUGS','CANCER_ASSOCIATIONS','DBSNP','COSMIC','PROTEIN_DOMAIN')
  ...
  for(v in c('ONCOGENE','TUMOR_SUPPRESSOR','NETWORK_CG','CANCER_PREDISPOSITION')){
