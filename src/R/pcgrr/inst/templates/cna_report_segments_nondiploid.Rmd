...
```{r cna_plot_custom, child='cna_plot.Rmd', eval = eval_cna_plot}
```
...
* <span style="color:#009E73"><b>Copy number amplifications</b></span>: Log(2) ratio >= `r report_data$pcgr_config$cna$logR_gain`
* <span style="color:red"><b>Homozygous deletions</b></span>: Log(2) ratio <= `r report_data$pcgr_config$cna$logR_homdel`

A total of __`r nrow(report_data$cna_data$ranked_segments)`__ unfiltered aberration segments satisfied the above criteria.
...
if(nrow(report_data$cna_data$ranked_segments) > 0){
...
if(nrow(report_data$cna_data$ranked_segments) > 0){
  report_data$cna_data$ranked_segments <- dplyr::rename(report_data$cna_data$ranked_segments, Segment = SEGMENT, Length_Mb = SEGMENT_LENGTH_MB, Cytoband = cytoband, Event_type = event_type)
  cna_segments_sd <- crosstalk::SharedData$new(report_data$cna_data$ranked_segments)
}else{
  cna_segments_sd <- crosstalk::SharedData$new(report_data$cna_data$ranked_segments)
}
...
if(nrow(report_data$cna_data$ranked_segments) >= 10){
...
crosstalk::bscols(
  ...
  DT::formatStyle('LogR',color='white', backgroundColor = DT::styleInterval(report_data$pcgr_config$cna$logR_homdel, c('red', '#009E73')))
)
