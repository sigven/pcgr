...
A total of __n = `r nrow(report_data$mutational_signatures_data$call_set)`__ SNVs were used for the mutational signature analysis of this tumor.
...
deconstructSigs::plotSignatures(report_data$mutational_signatures_data$whichSignatures_object)
```

```{r sigpie, echo=F, fig.width=12,fig.height = 6, dpi=200}
deconstructSigs::makePie(report_data$mutational_signatures_data$whichSignatures_object)
```
...
DT::datatable(report_data$mutational_signatures_data$cancertypes_aetiologies,options = myOptions,extensions=c("Buttons","Responsive"))
```
<br>
