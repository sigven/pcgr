...
```{r}
flexdashboard::valueBox(pcg_report$content$snv_indel$variant_statistic$n_tier3, 
...
flexdashboard::valueBox(NROW(pcg_report$content$snv_indel$variant_display$tier3$proto_oncogene), 
                        color = pcg_report$metadata$color_value_box, icon = "fa-dna")
```
...
n_tsgene <- 
  ...
  dplyr::filter(TIER == "TIER 3" & TUMOR_SUPPRESSOR == T & ONCOGENE == F) %>%
  nrow()
flexdashboard::valueBox(n_tsgene, color = pcg_report$metadata$color_value_box, icon = "fa-dna")
```
...
n_dual_role <- 
  pcg_report$content$snv_indel$variant_set$tsv %>% 
  dplyr::filter(TIER == "TIER 3" & TUMOR_SUPPRESSOR == T & ONCOGENE == T) %>%
  nrow() 
flexdashboard::valueBox(n_dual_role, color = pcg_report$metadata$color_value_box, icon = "fa-dna")
```
...
```{r table_browse_tier3, echo=F, results = "asis", eval = pcg_report$content$snv_indel$variant_statistic$n_tier3 > 0}

tier3_vars <- dplyr::bind_rows(pcg_report[['content']][['snv_indel']][['variant_display']][['tier3']][['tumor_suppressor']],
                              pcg_report[['content']][['snv_indel']][['variant_display']][['tier3']][['proto_oncogene']])
...
```{r tier3_missing_filters, echo=F, results = 'asis', eval = pcg_report$content$snv_indel$variant_statistic$n_tier3 == 0}
...
```{r table_tier3, eval = pcg_report$content$snv_indel$variant_statistic$n_tier3 > 0}
...
```{r tier3_missing_data, echo=F, results = 'asis', eval = pcg_report$content$snv_indel$variant_statistic$n_tier3 == 0}
cat('\n*  <i> <font style="font-size: 110%"><b>No</b> other variants found in proto-oncogenes or tumor suppressor genes.</font></i>', sep = '\n')
cat('\n')
```
