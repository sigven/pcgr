...
```{r}
#markers <- paste(unique(pcg_report$content$cna$variant_display$tier2$SYMBOL),collapse="|")
#if(stringr::str_count(markers, pattern="\\|") > 1){
  #markers <- paste0(paste(head(unique(pcg_report$content$cna$variant_display$tier2$SYMBOL), 2), collapse = "|"), "++")
#}
flexdashboard::valueBox(length(unique(pcg_report$content$cna$variant_display$tier2$SYMBOL)), color = pcg_report$metadata$color_value_box)
```

### Biomarker aberrations

```{r}
flexdashboard::valueBox(length(unique(pcg_report$content$cna$variant_display$tier2$SEGMENT)), color = pcg_report$metadata$color_value_box, icon = "fa-dna")
```
...
```{r table_browse_scna_tier2, echo=F, results = "asis", eval = NROW(pcg_report$content$cna$variant_display$tier2) > 0}
...
```{r tier2_scna_missing_filters, echo=F, results = 'asis', eval = NROW(pcg_report$content$cna$variant_display$tier2) == 0}
...
```{r table_scna_tier2, eval = NROW(pcg_report$content$cna$variant_display$tier2) > 0}
...
```{r tier2_scna_missing_data, echo=F, results = 'asis', eval = NROW(pcg_report$content$cna$variant_display$tier2) == 0}
cat('\n*  <i> <font style="font-size: 110%"><b>No</b> somatic copy-number aberrations of potential clinical significance found.</font></i>',sep='\n')
cat('\n')
```
