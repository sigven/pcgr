...
if(NROW(pcg_report$content$cna$variant_display$tier1) > 0){
  num_genes <- length(unique(pcg_report$content$cna$variant_display$tier1$SYMBOL))
}
...
```{r}
flexdashboard::valueBox(length(unique(pcg_report$content$cna$variant_display$tier1$SEGMENT)), color = pcg_report$metadata$color_value_box, icon = "fa-dna")
```
...
```{r table_browse_scna_tier1, echo=F, results = "asis", eval = NROW(pcg_report$content$cna$variant_display$tier1) > 0}
...
```{r tier1_scna_missing_filters, echo=F, results = 'asis', eval = NROW(pcg_report$content$cna$variant_display$tier1) == 0}
...
```{r table_scna_tier1, eval = NROW(pcg_report$content$cna$variant_display$tier1) > 0}
...
```{r tier1_scna_missing_data, echo=F, results = 'asis', eval = NROW(pcg_report$content$cna$variant_display$tier1) == 0}
cat('\n*  <i> <font style="font-size: 110%"><b>No</b> somatic copy-number aberrations of strong clinical significance found.</font></i>',sep='\n')
cat('\n')
```
