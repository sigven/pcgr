...
flexdashboard::valueBox(NROW(pcg_report$content$cna$variant_display$oncogene_gain),
                        color = ifelse(NROW(pcg_report$content$cna$variant_display$oncogene_gain) > 0,
...
flexdashboard::valueBox(NROW(pcg_report$content$cna$variant_display$tsgene_loss),
                        color = ifelse(NROW(pcg_report$content$cna$variant_display$tsgene_loss) > 0,
                                       pcg_report$metadata$color_value_box,
                                       pcg_report$metadata$color_none),
...
flexdashboard::valueBox(NROW(pcg_report$content$cna$variant_display$other_target),
                        color = ifelse(NROW(pcg_report$content$cna$variant_display$other_target) > 0,
                                       pcg_report$metadata$color_value_box,
                                       pcg_report$metadata$color_none),
...
```{r table_browse_scna_oncogene, echo=F, results = "asis", eval = NROW(pcg_report$content$cna$variant_display$oncogene_gain) > 0}

cat('\nProto-oncogenes subject to amplifications (right panel) can be filtered according to various criteria:\n')

drug_levels <- levels(as.factor(pcg_report[["content"]][["cna"]][["variant_display"]][["oncogene_gain"]]$DRUGS_ON_LABEL))
# if(length(drug_levels) > 0){
#   tier3_vars <- tier3_vars %>%
#     dplyr::arrange(MUTATION_HOTSPOT, desc(OPENTARGETS_RANK))
# }

oncogene_gain_variants <- crosstalk::SharedData$new(pcg_report[["content"]][["cna"]][["variant_display"]][["oncogene_gain"]])
crosstalk::bscols(
  list(
...
```{r oncogene_missing_filters, echo=F, results = 'asis', eval = NROW(pcg_report$content$cna$variant_display$oncogene_gain) == 0}
...
```{r table_scna_oncogene, eval = NROW(pcg_report$content$cna$variant_display$oncogene_gain) > 0}
...
```{r oncogene_missing_data, echo=F, results = 'asis', eval = NROW(pcg_report$content$cna$variant_display$oncogene_gain) == 0}
cat('\n*  <i> <b>No</b> proto-oncogenes subject to copy number amplifications.</i>',sep='\n')
cat('\n')
```
