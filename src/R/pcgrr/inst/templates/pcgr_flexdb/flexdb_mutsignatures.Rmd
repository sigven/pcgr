...
if(pcg_report$content$snv_indel$variant_statistic$n_snv < 100){
  col <- pcg_report[['metadata']][['color_palette']][['warning']][['values']][1]
}
flexdashboard::valueBox(pcg_report$content$snv_indel$variant_statistic$n_snv, color = col, icon = "fa-dna")
```
...
cat(paste0('-   <b>NOT DETERMINED (ND)</b>: The number of SNV calls (n = ',nrow(pcg_report[['content']][['m_signature_mp']][['variant_set']][['all']]),') is too limited. A minimum of ',pcg_report[['metadata']][['config']][['migs']][['mutation_limit']],' SNVs is currently required for a mutational signature analysis to be performed.'), sep ="\n")
