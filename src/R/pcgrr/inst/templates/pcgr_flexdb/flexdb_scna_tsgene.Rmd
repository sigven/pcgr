...
```{r table_browse_scna_tsgene, echo=F, results = "asis", eval = NROW(pcg_report$content$cna$variant_display$tsgene_loss) > 0}

cat('\nTumor suppressor genes subject to homozygous deletions (right panel) can be filtered according to various criteria:\n')

tsgene_loss_variants <- crosstalk::SharedData$new(pcg_report[["content"]][["cna"]][["variant_display"]][["tsgene_loss"]])
crosstalk::bscols(
  list(
...
```{r tsgene_missing_filters, echo=F, results = 'asis', eval = NROW(pcg_report$content$cna$variant_display$tsgene_loss) == 0}
...
```{r table_scna_tsgene, eval = NROW(pcg_report$content$cna$variant_display$tsgene_loss) > 0}
...
```{r tsgene_missing_data, echo=F, results = 'asis', eval = NROW(pcg_report$content$cna$variant_display$tsgene_loss) == 0}
cat('\n*  <i> <b>No</b> tumor suppressor genes subject to homozygous deletions.</i>',sep='\n')
cat('\n')
```
