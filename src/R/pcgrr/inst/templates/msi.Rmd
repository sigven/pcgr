...
* _Predicted MSI status for `r pcg_report$metadata$sample_name`_: __`r pcg_report[['content']][['msi']][['prediction']][['msi_stats']][['predicted_class']]`__
...
if(NROW(pcg_report[['content']][['msi']][['prediction']][['mmr_pol_variants']]) > 0){
  DT::datatable(pcg_report[['content']][['msi']][['prediction']][['mmr_pol_variants']], escape=F,extensions=c("Buttons","Responsive"), width = "100%",options=list(buttons = c('csv','excel'),dom = 'Bfrtip'))
}else{
