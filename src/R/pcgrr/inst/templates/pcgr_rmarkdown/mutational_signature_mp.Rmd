...
cat('Specifically, for tumors of type <b>', pcg_report$metadata$config$tumor_properties$tumor_type,'</b>, mutational signature reconstruction is here limited to the following reference collection:<br>',sep="")
...
```{r results = "asis", include = !missing_data_norun & pcg_report$metadata$config$msigs$all_reference_signatures}
cat("For this analysis, '--all_reference_signatures' have been turned on, which means that all reference signatures (n = 67) have been considered during optimal reconstruction of the mutational profile with reference signatures",sep="\n")
htmltools::br()
...
ggplot2::ggplot(plot_data, aes(x = 2, y = prop_group, fill = group)) +
