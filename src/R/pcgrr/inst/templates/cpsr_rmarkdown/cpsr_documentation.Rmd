...
for(n in names(cps_report$metadata$pcgr_db_release)){
  if(n == 'mosdepth' | n == 'mutationalpatterns' | n == 'vcf2maf' | n == 'maftools'){
    next
  }
...
#### Assignment of numerical pathogenicity scores to pathogenicity levels

How do we derive the variant classification (<b>CPSR_CLASSIFICATION</b>) from the aggregated pathogenicity score (<b>CPSR_CLASSIFICATION_SCORE</b>)?

Based on a classification benchmark against ClinVar-classified variants (with a review status of minimum two stars) in cancer predisposition genes, the following thresholds are currently used to convert numeric classification scores to categorical tiers:

<br>

`r reactable::reactable(pcgrr::cpsr_acmg_score2tier, defaultColDef = reactable::colDef(html = T))`
