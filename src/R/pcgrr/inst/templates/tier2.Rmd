...
if(NROW(pcg_report[['snv_indel']][['variant_display']][['tier2']][['hotspot']]) > 0){
  ...
  DT::datatable(dplyr::select(pcg_report[['snv_indel']][['variant_display']][['tier2']][['hotspot']], CANCER_MUTATION_HOTSPOT,dplyr::everything()), options = myOptions, escape=F, extensions = c("Buttons","Responsive")) %>%
...
if(NROW(pcg_report[['snv_indel']][['variant_display']][['tier2']][['curated_mutation']]) > 0){
  ...
  curated_mutations <- dplyr::select(pcg_report[['snv_indel']][['variant_display']][['tier2']][['curated_mutation']], -CANCER_MUTATION_HOTSPOT)
...
if(NROW(pcg_report[['snv_indel']][['variant_display']][['tier2']][['predicted_driver']]) > 0){
  ...
  predicted_drivers <- dplyr::select(pcg_report[['snv_indel']][['variant_display']][['tier2']][['predicted_driver']], -c(INTOGEN_DRIVER_MUT, CANCER_MUTATION_HOTSPOT, DOCM_DISEASE, DOCM_LITERATURE))
