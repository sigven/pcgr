...
from dgg_engine import pcgr_vars, arg_checker, utils, cna
from dgg_engine.utils import getlogger, check_subprocess, remove_file, random_id_generator
from dgg_engine.config import populate_config_data, create_config
from dgg_engine.maf import update_maf
from dgg_engine.vep import get_vep_command
from dgg_engine.expression import parse_expression, integrate_variant_expression, correlate_sample_expression
from dgg_engine.expression import find_expression_outliers, aggregate_tpm_per_cons
from dgg_engine.variant import clean_annotations, set_allelic_support, append_annotations, calculate_tmb
...
def cli():
    ...
    optional_germline.add_argument("--input_cpsr_yaml", dest="input_cpsr_yaml", help="CPSR YAML configuration file (file '<cpsr_sample_id>.cpsr.<genome_assembly>.conf.yaml')")
    optional_germline.add_argument("--cpsr_ignore_vus", action="store_true", help="Do not show variants of uncertain significance (VUS) in the germline section of the HTML report (default: %(default)s)")
...
def run_pcgr(input_data, output_data, conf_options):
    ...
    if not conf_options['other']['no_reporting'] and not input_vcf == 'None':
        ...
        dgg_engine_r_script = utils.script_path(pcgr_conda, 'bin/dgg_engine_r.R')
        export_pcgrr = utils.pcgrr_conda_env_export(pcgrr_conda)
        pcgr_report_command = (
                 f"{export_pcgrr} && {rscript} {dgg_engine_r_script} {yaml_fname} {quarto_env_vars}")
