...
def verify_refdata(arg_dict: dict, logger = None, cpsr = False):
    ...
    if compliant_data_bundle == 0:
        err_msg = (
            f'The DGG Engine reference bundle is not compliant with the software version - please download the '
            f'latest software and reference bundle (see https://github.com/LauferVA/dgg_rules_somatic for instructions)')
        if cpsr is False:
            err_msg = (
                f'The DGG Engine reference bundle is is not compliant with the software version - please download the ',
