...
from dgg_engine import pcgr_vars, arg_checker, utils, config, variant
from dgg_engine.utils import check_subprocess, getlogger, error_message, warn_message, remove_file, random_id_generator
from dgg_engine.config import populate_config_data
...
def get_args():
    ...
    required.add_argument('--refdata_dir',help=f"Directory that contains the DGG Engine/CPSR reference data, e.g. ~/dgg_engine-data-{pcgr_vars.PCGR_VERSION}", required = True)
