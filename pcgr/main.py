...
def cli():
    ...
    optional_tumor_only.add_argument("--maf_onekg_eur", dest="maf_onekg_eur", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct (1000 Genomes Project - European pop, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_onekg_amr", dest="maf_onekg_amr", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct (1000 Genomes Project - Ad Mixed American pop, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_onekg_afr", dest="maf_onekg_afr", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct (1000 Genomes Project - African pop, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_onekg_eas", dest="maf_onekg_eas", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct (1000 Genomes Project - East Asian pop, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_onekg_sas", dest="maf_onekg_sas", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct (1000 Genomes Project - South Asian pop, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_onekg_global", dest="maf_onekg_global", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct (1000 Genomes Project - global pop, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_gnomad_nfe", dest="maf_gnomad_nfe", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct, (gnomAD - European (non-Finnish), default: %(default)s)")
    optional_tumor_only.add_argument("--maf_gnomad_asj", dest="maf_gnomad_asj", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct, (gnomAD - Ashkenazi Jewish, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_gnomad_fin", dest="maf_gnomad_fin", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct, (gnomAD - European (Finnish), default: %(default)s)")
    optional_tumor_only.add_argument("--maf_gnomad_oth", dest="maf_gnomad_oth", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct, (gnomAD - Other, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_gnomad_amr", dest="maf_gnomad_amr", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct, (gnomAD - Latino/Admixed American, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_gnomad_afr", dest="maf_gnomad_afr", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct, (gnomAD - African/African-American, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_gnomad_eas", dest="maf_gnomad_eas", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct, (gnomAD - East Asian, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_gnomad_sas", dest="maf_gnomad_sas", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct, (gnomAD - South Asian, default: %(default)s)")
    optional_tumor_only.add_argument("--maf_gnomad_global", dest="maf_gnomad_global", type=float, default=0.002, help="Exclude variants in tumor (SNVs/InDels, tumor-only mode) with MAF > pct, (gnomAD - global population, default: %(default)s)")
...
def run_pcgr(arg_dict, pcgr_paths, config_options):
    ...
    if not input_vcf == 'None':
        ...
        vep_bgzip_command = f'bgzip -f -c {vep_vcf} > {vep_vcf}.gz'
        ...
        # TODO: use 'os.remove()' instead of 'rm -f'
        clean_command = (
                f'rm -f {vep_vcf}* {vep_vcfanno_annotated_vcf} '
