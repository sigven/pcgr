...
### News
* _November 21st 2017_: **0.5.1.2 release**
  * PASS variants fetched properly
* _November 17th 2017_: **0.5.1.1 release**
  * Updated data bundle due to ClinVar bug (variants with empty alleles)
  * added VEP option (_failed_) to capture all annotations
...
### Annotation resources included in PCGR (v0.5.1.2)
...
1. Download and unpack the [latest software release (0.5.1.2)](https://github.com/sigven/pcgr/releases/tag/0.5.1.2)
2. Download and unpack the data bundle (approx. 16Gb) in the PCGR directory
   * Download [the accompanying data bundle](https://drive.google.com/file/d/1NSeMWpLVMBcCEDYpOLsuWSnKfZEaamip/) from Google Drive to `~/pcgr-X.X` (replace _X.X_ with the version number, e.g `~/pcgr-0.5.1.2`)
...
3. Pull the [PCGR Docker image (0.5.1.2)](https://hub.docker.com/r/sigven/pcgr/) from DockerHub (approx 4.2Gb):
...
` ~/pcgr-0.5.1.2 ~/pcgr-0.5.1.2/examples ~/pcgr-0.5.1.2/examples/pcgr_configuration.toml tumor_sample.COAD`
